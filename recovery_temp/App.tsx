...
function App() {
  ...
  const fetchDashboardData = async (userId: string, isInitial: boolean = false) => {
    try {
      ...
      const [userRes, sessionRes, allSessionsRes, skillsRes] = await Promise.all([
      ...
      setUserSongs(safeSkills.map((p: any) => {
      ...
      const [wallRes, membersRes, formingBandsRes] = await Promise.all([
        ...
        supabase.from('bands').select('*, band_songs(*, band_song_slots(*))').eq('status', 'forming').eq('school_id', userData.school_id)
      ]);
      ...
      if (formingBands.length > 0) {
        ...
        wallData.forEach((song: any) => {
          ...
          ['starter', 'original'].forEach(level => {
            ...
            // Filter out musicians who are already in a band slot for this song
            const availableMusicians = levelSkills.filter((skill: any) => {
              const isTaken = (allMembers || []).some((m: any) => {
                ...
                // Check if they occupy a slot for this specific song
                const hasSlot = (bandData.band_songs || []).some((bs: any) => {
                  if (bs.song_id !== song.id) return false;
                  return (bs.band_song_slots || []).some((slot: any) => slot.user_id === skill.user_id);
                });

                // Check if it's the band's primary song
                const isMainSong = bandData.song_id === song.id;

                return isMainSong || hasSlot;
              });
              return !isTaken;
                                ...
                                {canJoin && (
                                  <button 
                                    onClick={async () => {
                                      ...
                                      // Force immediate refresh to trigger the founding detection
                                      fetchDashboardData(user.id);
                                    }}
                              ...
                              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
                               ...
                               {form.isComplete && isMySlot && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '24px' }}>
                                  ...
                                  <button 
                                    onClick={() => {
                                      const myReadySkill = userSongs.find(us => us.song_id === song.song_id && us.is_stage_ready);
                                      if (myReadySkill) {
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      {(() => {
                        const vocalOpportunities = allBands.filter(band => {
                          ...
                          // Vocal check: Is a vocalist actually requested?
                          if (!normalizedReq['Vocals'] || normalizedReq['Vocals'] === 0) return false;

                          // Instrumental Check: Are all other required positions filled?
                          ...
                          const neededVocalists = normalizedReq['Vocals'] || 1;
                          return currentVocalists < neededVocalists;
                        });
