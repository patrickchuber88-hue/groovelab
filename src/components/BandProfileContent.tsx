...
const BandProfileContent: React.FC<BandProfileContentProps> = ({
  ...
  // Helper to check if a song is fully occupied by the band's current members
  const checkIfFull = (song: any) => {
    const instrumentation = song?.instrumentation || {};
    const bandMembers = selectedBandForProfile?.band_members || [];
    return Object.entries(instrumentation).every(([inst, count]) => {
      const normReq = normalize(inst);
      if (normReq === 'Vocals') return true;
      const membersWithInst = bandMembers.filter((m: any) => normalize(m.instrument) === normReq).length;
      return membersWithInst >= (count as number);
    });
  };

  // Filtered repertoire: only songs where ALL required instrument slots are occupied by MASTERED users
  // OR songs that are fully occupied (assigned) by the band members
  const repertoireSongs = useMemo(() => {
    return allSongs.filter(song => {
      ...
      // 3. Check if it's "full" (all slots assigned to current members)
      if (checkIfFull(song)) return true;

      // 4. Check latest proposal data for mastery (original logic)
      const prop = songProposals.find((p: any) => p.song_id === song.id);
      if (!prop && (!bandSong || bandSong.status !== 'active')) return false;
      ...
      return Object.entries(instrumentation).every(([inst, count]) => {
        ...
        const masteredMembersWithInst = (selectedBandForProfile.band_members || []).filter((m: any) => {
          if (normalize(m.instrument) !== normReq) return false;
          const userObj = Array.isArray(m.users) ? m.users[0] : m.users;
          const skills = userObj?.user_song_skills || [];
          const skill = skills.find((sk: any) => sk.song_id === song.id && normalize(sk.instrument) === normReq);
          return skill?.is_stage_ready || skill?.progress_percent >= 100;
        }).length;

        return masteredMembersWithInst >= (count as number);
      });
    });
  }, [allSongs, selectedBandForProfile.song_id, selectedBandForProfile.band_members, bSongs, songProposals]);
                        ...
                        <div style={{ 
                          ...
                          {(() => {
                            ...
                            return slots.map((slot, idx) => {
                              ...
                              const uFromBand = member?.users ? (Array.isArray(member.users) ? member.users[0] : member.users) : null;
                              
                              const uFromSlot = slot.occupant?.users ? (Array.isArray(slot.occupant.users) ? slot.occupant.users[0] : slot.occupant.users) : null;
                              
                              const u = uFromSlot || uFromBand;
                              ...
                              return (
                                <div key={idx} style={{ width: '65px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                                  <div style={{ 
                                    width: '48px', height: '48px', borderRadius: '50%',
                                    background: u ? 'white' : 'rgba(255,255,255,0.03)', 
                                    border: (isMe || isMastered) ? `2px solid #ef4444` : (u ? 'none' : '1.5px dashed rgba(255,255,255,0.1)'),
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
                                    filter: u && !isMastered ? 'grayscale(100%)' : 'none', opacity: u && !isMastered ? 0.6 : 1
                                  }}>
                                    {u ? (
                                      ...
                                      <div style={{ fontSize: '1rem', opacity: 0.2 }}>{APP_INSTRUMENT_ICONS[slot.inst] || <Plus size={16} />}</div>
                                    )}
                                  </div>
                                  <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px', width: '100%' }}>
                                    <div style={{ fontSize: '0.6rem', fontWeight: 950, color: 'white', opacity: u ? 1 : 0.4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: '100%' }}>
                   ...
                   <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {repertoireSongs.length === 0 ? (
                        ...
                        repertoireSongs.map((song: any, idx: number) => {
                               // 1. Vocalists (from band members or song slots)
                               const bandMembers = selectedBandForProfile.band_members || [];
                               const songVocalists = bandMembers.filter((m: any) => {
                                 return (m.instrument || '').toLowerCase().includes('vocal') || (m.instrument || '').toLowerCase().includes('gesang');
                               });
                               
                               const isTeacher = user?.role === 'admin' || user?.role === 'teacher' || selectedBandForProfile.coach_id === user?.id;

                               // 2. Guest Musicians & Missing Instruments
                               const instReq = song.instrumentation || { 'E-Gitarre': 1, 'E-Bass': 1, 'E-Drums': 1 };
                               const bandSongId = bSongs.find((bs: any) => bs.song_id === song.id)?.id;
                               const filledSlots = bSongs.find((bs: any) => bs.song_id === song.id)?.band_song_slots || [];
                               
                               const coreMemberIds = bandMembers.map((m: any) => m.user_id);
                               const guestMusicians = filledSlots.filter((s: any) => s.user_id && !coreMemberIds.includes(s.user_id) && !(s.instrument || '').toLowerCase().includes('vocal') && !(s.instrument || '').toLowerCase().includes('gesang'));

                               const missingInstruments: string[] = [];
                               Object.entries(instReq).forEach(([inst, count]) => {
                                 ...
                                 // Count core members
                                 filledCount += bandMembers.filter((m: any) => m.user_id && checkInstMatch(m.instrument)).length;
                                 
                                 // Count guest musicians
                                 filledCount += guestMusicians.filter((gm: any) => gm.user_id && checkInstMatch(gm.instrument)).length;

                                 for(let i=0; i < (count as number) - filledCount; i++) missingInstruments.push(inst);
                               });

                               const hasExtras = songVocalists.length > 0 || guestMusicians.length > 0 || missingInstruments.length > 0;

                               return (
                                 <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '16px', borderRadius: '24px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.04)', transition: 'all 0.2s', flexWrap: 'wrap' }}>
                                   ...
                                   {hasExtras && (
                                     <div style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'rgba(255,255,255,0.02)', padding: '6px 14px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)', flexWrap: 'wrap' }}>
                                       
                                       {songVocalists.length > 0 && (
                                         <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                           <div style={{ fontSize: '0.65rem', fontWeight: 950, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Gesang:</div>
                                           {songVocalists.map((vm: any, vIdx: number) => {
                                             const vu = vm.users ? (Array.isArray(vm.users) ? vm.users[0] : vm.users) : null;
                                             const vid = vu?.id || vm.user_id;
                                             const canRemove = user && (user.id === vid || user.role === 'admin' || user.role === 'teacher' || selectedBandForProfile.coach_id === user.id);
                                             
                                             return (
                                               <div key={`voc-${vIdx}`} style={{ display: 'flex', alignItems: 'center', gap: '8px', position: 'relative', paddingRight: canRemove && !isSharedView ? '36px' : '0' }}>
                                                 <img src={vu?.photo_url || "/avatar_ghost.jpg"} style={{ width: '40px', height: '40px', borderRadius: '50%', border: '2px solid rgba(255,255,255,0.1)', objectFit: 'cover' }} />
                                                 <span style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                   <span style={{ fontSize: '0.8rem', fontWeight: 950, color: 'white' }}>{vu?.first_name || vm.external_name}</span>
                                                 </span>
                                                 {canRemove && !isSharedView && (
                                                   <button onClick={() => handleRemoveMember(vid, vu?.first_name || vm.external_name || 'Gesang', [vm.instrument])} style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '50%', width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                                                     <X size={12} />
                                                   </button>
                                                 )}
                                               </div>
                                       ...
                                       {guestMusicians.length > 0 && (
                                         <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: songVocalists.length > 0 ? '16px' : '0' }}>
                                           <div style={{ fontSize: '0.65rem', fontWeight: 950, color: brandColor, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Gäste:</div>
                                           {guestMusicians.map((gm: any, gIdx: number) => {
                                             const gu = gm.users ? (Array.isArray(gm.users) ? gm.users[0] : gm.users) : null;
                                             const gid = gu?.id || gm.user_id;
                                             const canRemove = user && (user.id === gid || user.role === 'admin' || user.role === 'teacher' || selectedBandForProfile.coach_id === user.id);
                                             
                                             return (
                                               <div key={`guest-${gIdx}`} style={{ display: 'flex', alignItems: 'center', gap: '6px', position: 'relative', paddingRight: canRemove && !isSharedView ? '32px' : '0' }}>
                                                 <img src={gu?.photo_url || "/avatar_ghost.jpg"} style={{ width: '32px', height: '32px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', objectFit: 'cover' }} />
                                                 <span style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                   <span style={{ fontSize: '0.75rem', fontWeight: 950, color: 'white' }}>{gu?.first_name || gm.external_name}</span>
                                                   <span style={{ fontSize: '0.55rem', fontWeight: 950, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase' }}>{gm.instrument}</span>
                                                 </span>
                                                 {canRemove && !isSharedView && (
                                                   <button onClick={async () => {
                                                      if(window.confirm('Gastmusiker entfernen?')) {
                                                         await supabase.from('band_song_slots').delete().eq('id', gm.id);
                                                         if(onRefresh) onRefresh(); else window.location.reload();
                                                      }
                                                   }} style={{ position: 'absolute', right: 0, top: '50%', transform: 'translateY(-50%)', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '50%', width: '20px', height: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                                                     <X size={10} />
                                                   </button>
                                                 )}
                                               </div>
                                       ...
                                       {missingInstruments.length > 0 && !isSharedView && (
                                         <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: (songVocalists.length > 0 || guestMusicians.length > 0) ? '16px' : '0' }}>
                                           <div style={{ fontSize: '0.65rem', fontWeight: 950, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Offen:</div>
                                           {missingInstruments.map((mi, mIdx) => (
                                             <button 
                                                key={`miss-${mIdx}`} 
                                                onClick={async () => {
                                                  if (!isTeacher) return;
                                                  const guestName = window.prompt(`Name des Gastmusikers für ${mi} eingeben:`);
                                                  ...
                                                  try {
                                                    ...
                                                    alert("Fehler beim Hinzufügen des Gastmusikers.");
                                                  }
                                                }}
                                                title={isTeacher ? `Gastmusiker für ${mi} hinzufügen` : `Fehlt: ${mi}`}
                                                style={{ 
                                                ...
                                                onMouseEnter={(e) => { if(isTeacher) e.currentTarget.style.background = 'rgba(239,68,68,0.2)'; }}
                                                onMouseLeave={(e) => { if(isTeacher) e.currentTarget.style.background = 'rgba(239,68,68,0.1)'; }}
                                             >
                                               {isTeacher ? `+ ${mi} (Gast)` : mi}
                                             </button>
                                           ))}
                                         </div>
                                       )}
                   ...
                   <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        {(() => {
                           ...
                           // Filter: Members who ONLY sing are NOT displayed in the sidebar
                           const visibleMembers = Object.values(grouped).filter((m: any) => {
                             const onlySings = m.instruments.every((inst: string) => (inst || '').toLowerCase().includes('vocal') || (inst || '').toLowerCase().includes('gesang'));
                             return !onlySings;
                           });
                           ...
                           return visibleMembers.map((m: any, idx: number) => {
                             ...
                             // We don't show "& Gesang" in the sidebar anymore as requested
                             const displayInst = otherInsts.join(' & ');
                                ...
                                >
                                   ...
                                   <div style={{ flex: 1 }}>
                                      ...
                                      <div style={{ fontSize: "0.65rem", fontWeight: 950, color: brandColor, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                                        {APP_INSTRUMENT_ICONS[otherInsts[0]] || "🎸"} {displayInst}
                                      </div>
                                   </div>
                                   {canRemove && !isSharedView && (
                                     <button
                                       onClick={() => handleRemoveMember(uid, u?.first_name || m.external_name || 'Mitglied', otherInsts)}
