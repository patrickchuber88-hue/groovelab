...
import { Monitor, Music, Award, Box, Plus, AlertCircle, User, Users, Star, TrendingUp, Shield, Zap, Play, Info, CheckCircle, Check, Search, Trash2, Bell, X, Clock } from 'lucide-react';
...
const StationNode = React.memo(({ num, color, inst, sess, isMe, viewMode, onProfileSelect, onLogout }: { 
  num: number, color: string, inst: string, sess: any, isMe: boolean, viewMode: string, onProfileSelect: (u: any) => void, onLogout: (id: string) => void 
}) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (tData?.school_id) {
        ...
        const activeCoaches = (allCoaches || []).filter(c => trulyActive.some(s => s.user_id === c.id));
        ...
        // 8. Help
        const { data: helpData } = await supabase.from('help_requests').select('*, users(*)').eq('school_id', tData.school_id).eq('status', 'open').order('created_at', { ascending: false });
        ...
        (wallData || []).forEach(song => {
          ...
          ['starter', 'original'].forEach(level => {
            ...
            songFormations.forEach(({ groupKey, members }) => {
              ...
              Object.entries(instrumentation).forEach(([i, c]) => {
                if (i.toLowerCase().includes('vocals') || i.toLowerCase().includes('gesang')) return;
                ...
                for(let k=0; k < (c as number) - filledForInst; k++) {
                  let norm = i;
                  if (i.toLowerCase().includes('guitar')) norm = 'Gitarre';
                  else if (i.toLowerCase().includes('drum')) norm = 'Drums';
                  else if (i.toLowerCase().includes('bass')) norm = 'Bass';
                  else if (i.toLowerCase().includes('piano') || i.toLowerCase().includes('keys')) norm = 'Piano';
        ...
        const { data: formingBands } = await supabase
          .from('bands')
          .select('*, songs(*), band_songs(*, songs(*), band_song_slots(*, users!user_id(*)))')
          .eq('school_id', tData.school_id)
          .eq('status', 'forming');
          
        const bandFormations = (formingBands || []).map(b => {
          const mainSong = b.band_songs?.[0];
          const songObj = mainSong?.songs || b.songs || { title: b.name, artist: 'Unbekannte Band' };
          ...
          const missingInstruments: string[] = [];
          Object.entries(inst).forEach(([i, c]) => {
            if (i.toLowerCase().includes('vocals') || i.toLowerCase().includes('gesang')) return;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '32px', alignItems: 'start' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '32px' }}>
            {[3, 4, 5, 6].map(n => {
              ...
              return (
                <StationNode key={n} num={n} color={getStationColor(n)} inst={n < 5 ? 'E-Piano' : 'E-Drum'} sess={sess} isMe={sess?.user_id === userId} viewMode={viewMode} onProfileSelect={setSelectedStudentProfile} onLogout={handleLogoutStudent} />
              );
            })}
            ...
            <div style={{ gridColumn: '1', display: 'flex', flexDirection: 'column', gap: '24px' }}>
              {[2, 1].map(n => {
                ...
                return <StationNode key={n} num={n} color="#ef4444" inst="E-Gitarre" sess={sess} isMe={sess?.user_id === userId} viewMode={viewMode} onProfileSelect={setSelectedStudentProfile} onLogout={handleLogoutStudent} />;
              })}
            </div>
            ...
            <div style={{ gridColumn: '4', display: 'flex', flexDirection: 'column', gap: '24px' }}>
              {[7, 8].map(n => {
                ...
                return <StationNode key={n} num={n} color="#eab308" inst="Bass" sess={sess} isMe={sess?.user_id === userId} viewMode={viewMode} onProfileSelect={setSelectedStudentProfile} onLogout={handleLogoutStudent} />;
              })}
            </div>
          </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                     {wallSongs.map((form: any, fIdx: number) => {
                       ...
                       order.forEach(instName => {
                         ...
                         for(let i=0; i < count; i++) allRequired.push(instName);
                       });
                       ...
                       return (
                         <div key={form.id} style={{ 
                           ...
                           <div style={{ display: 'flex', gap: '10px', flexWrap: 'nowrap' }}>
                             {allRequired.map((inst, idx) => {
                               // Accurate instrument-based fill check
                               const isFilled = form.members.some((m: any) => {
                                 const mi = (m.instrument || '').toLowerCase();
                                 const target = inst.toLowerCase();
                                 return mi.includes(target.replace('e-', '')) || target.includes(mi.replace('e-', ''));
                               });
                           ...
                           {form.missingInstruments.length > 0 && (
                             <div style={{ 
                               ...
                               GESUCHT: {form.missingInstruments[0].toUpperCase()}
                             </div>
                           )}
                         </div>
                       );
           ...
           <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
             {allBands.filter(b => b.name.toLowerCase().includes(bandSearch.toLowerCase())).map(band => (
               <div key={band.id} onClick={() => onOpenBandProfile?.(band)} className="glass-panel" style={{ padding: '24px', borderRadius: '24px', cursor: 'pointer' }}>
                 ...
                 <p style={{ color: '#64748b', fontSize: '0.8rem' }}>{band.band_members?.length} Mitglieder</p>
               </div>
             ))}
           </div>
        </div>
