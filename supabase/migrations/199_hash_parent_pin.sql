-- Migration 199: Cryptographic Hashing for Parental PIN
-- 1. Convert any existing plaintext 4-digit parent_pins to SHA-256 hashes
UPDATE public.users_raw 
SET parent_pin = encode(extensions.digest(parent_pin, 'sha256'), 'hex')
WHERE parent_pin IS NOT NULL 
  AND length(parent_pin) = 4 
  AND parent_pin ~ '^[0-9]+$';

-- 2. Update trigger function handle_users_view_dml to hash the parent_pin on insert/update
...
-- 3. Update secure server-side RPC verify function to compare SHA-256 hashes
...
DECLARE
    ...
    default_hash TEXT;
BEGIN
    SELECT parent_pin INTO stored_hash FROM public.users_raw WHERE id = student_id;
    input_hash := encode(extensions.digest(input_pin, 'sha256'), 'hex');
    default_hash := encode(extensions.digest('0000', 'sha256'), 'hex');

    RETURN (stored_hash = input_hash) 
        OR (stored_hash IS NULL AND input_pin = '0000') 
        OR (stored_hash = '' AND input_pin = '0000')
        OR (stored_hash = default_hash AND input_pin = '0000');
END;
