-- Migration 196: Parental controls and AVV signatures schema
-- 1. Add parent_pin column to users_raw
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS parent_pin TEXT DEFAULT NULL;

-- 2. Create school_contract_signatures table
CREATE TABLE IF NOT EXISTS public.school_contract_signatures (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    school_id UUID REFERENCES public.schools(id) ON DELETE CASCADE,
    admin_user_id UUID, -- References users_raw(id) but not forcing foreign key to avoid circular reference in trigger function
...
-- 3. Create parent_consent_logs table
CREATE TABLE IF NOT EXISTS public.parent_consent_logs (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    consent_type TEXT NOT NULL, -- e.g., 'direct_communication'
    ip_address TEXT,
    user_agent TEXT
);
...
DROP POLICY IF EXISTS "Users can see signatures from their own school" ON public.school_contract_signatures;
CREATE POLICY "Users can see signatures from their own school" ON public.school_contract_signatures
    FOR SELECT USING (school_id = (auth.jwt()->>'school_id')::uuid);

DROP POLICY IF EXISTS "Admins can create signatures for their own school" ON public.school_contract_signatures;
CREATE POLICY "Admins can create signatures for their own school" ON public.school_contract_signatures
    FOR INSERT WITH CHECK (school_id = (auth.jwt()->>'school_id')::uuid);

DROP POLICY IF EXISTS "Users can see consent logs from their own school" ON public.parent_consent_logs;
CREATE POLICY "Users can see consent logs from their own school" ON public.parent_consent_logs
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM public.users_raw u WHERE u.id = student_id AND u.school_id = (auth.jwt()->>'school_id')::uuid)
    );

DROP POLICY IF EXISTS "Users can create consent logs from their own school" ON public.parent_consent_logs;
CREATE POLICY "Users can create consent logs from their own school" ON public.parent_consent_logs
    FOR INSERT WITH CHECK (
        EXISTS (SELECT 1 FROM public.users_raw u WHERE u.id = student_id AND u.school_id = (auth.jwt()->>'school_id')::uuid)
    );

-- 5. Drop trigger first to recreate it
DROP TRIGGER IF EXISTS trg_users_view_dml ON public.users;

-- 6. Update handle_users_view_dml trigger function
...
-- 7. Recreate trigger on the view
