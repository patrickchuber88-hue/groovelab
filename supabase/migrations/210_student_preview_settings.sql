-- Migration: 210_student_preview_settings
-- Description: Add pin_enforced_for_preview to users_raw and mask last_name in users view for public QR scan previews.

-- 1. Add column to users_raw
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS pin_enforced_for_preview BOOLEAN DEFAULT false NOT NULL;

-- 2. Recreate public.users view with masked last_name and pin_enforced_for_preview column
...
-- 3. Recreate DML trigger function to handle pin_enforced_for_preview
...
BEGIN
    IF TG_OP = 'UPDATE' THEN
        -- Secure update checks (cannot update master admin flags unless master admin)
    ...
    ELSIF TG_OP = 'INSERT' THEN
        ...
        INSERT INTO public.users_raw (
            ...
            parent_pin, pin_enforced_for_preview
        ) VALUES (
