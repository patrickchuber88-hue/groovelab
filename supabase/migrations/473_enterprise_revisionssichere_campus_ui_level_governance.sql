-- ==============================================================================
-- Migration 473: Enterprise+ Revisionssichere Didaktische Alters-UI Governance
-- OWASP ASVS Level 3 / BGB § 104 / DSGVO Art. 8 / NIS-2 / ISO 27001
-- ==============================================================================

-- 1. Autoritative Härtung der save_parent_controls RPC
...
BEGIN
    ...
    -- Adulthood Parity (strictly from users_raw columns: age, birth_date)
    v_is_adult_user := (
    ...
    -- 🛡️ GOLDSTANDARD FAIL-CLOSED: Kein Schüler-Bypass erlaubt
    ...
    -- BGB Schutz: Junior (unter 11 J.) darf niemals eigenständig stornieren / verschieben
    ...
    -- Token-Version Inkrementierung (Cache-Busting auf allen Schüler-Instanzen)
    v_new_token_version := COALESCE(v_user.token_version, 1) + 1;
    ...
    UPDATE public.users_raw SET
        ...
        parent_allow_reschedule_confirm = CASE WHEN v_ui_level = 'junior' THEN FALSE ELSE parent_allow_reschedule_confirm END,
    ...
    -- Parallele Synchronisation in students (falls existent)
    BEGIN
    ...
    -- 5. Revisionssicheres Audit-Logging mit kryptographischem Auth-Vektor & Hash (Fail-Closed)
...
-- 2. Komfort-RPC: apply_parent_ui_level_governance
CREATE OR REPLACE FUNCTION public.apply_parent_ui_level_governance(
    p_student_id UUID,
    p_ui_level TEXT,
...
DECLARE
    v_settings JSONB;
BEGIN
    v_settings := jsonb_build_object('campus_ui_level', p_ui_level);
    IF p_parent_pin IS NOT NULL AND length(trim(p_parent_pin)) >= 4 THEN
        v_settings := v_settings || jsonb_build_object('parent_pin', trim(p_parent_pin));
    END IF;
    IF p_lease_token IS NOT NULL AND length(trim(p_lease_token)) >= 10 THEN
        v_settings := v_settings || jsonb_build_object('lease_token', trim(p_lease_token));
    END IF;

    RETURN public.save_parent_controls(p_student_id, v_settings);
END;
$$;

GRANT EXECUTE ON FUNCTION public.apply_parent_ui_level_governance(UUID, TEXT, TEXT, TEXT) TO authenticated, anon, service_role;
