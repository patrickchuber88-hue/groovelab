-- ==============================================================================
-- MIGRATION 357: DEDICATED CALENDAR TOKEN & SECURE REVOCATION RPC
-- ==============================================================================
-- 1. Add calendar_token to public.users_raw
ALTER TABLE public.users_raw 
ADD COLUMN IF NOT EXISTS calendar_token TEXT UNIQUE;

-- Create index for fast, O(1) lookup in Deno iCal feed
CREATE INDEX IF NOT EXISTS idx_users_raw_calendar_token 
ON public.users_raw(calendar_token) 
WHERE calendar_token IS NOT NULL;

-- 2. Update public.users view to expose calendar_token safely (Zero-Leakage)
...
-- 3. INSTEAD OF Trigger-Funktion für public.users wiederherstellen & härten
...
-- 4. RPC: get_or_rotate_calendar_token
-- Generates a 256-bit cryptographically secure token or rotates an existing one
...
BEGIN
    ...
    IF p_user_id IS NOT NULL THEN
        -- Authorization: Self, Master-Admin, or School Staff in same school
        ...
        ELSIF v_caller_id IS NOT NULL AND EXISTS (
            ...
            v_target_id := p_user_id;
        ELSE
            -- Fallback: If caller ID is null (e.g. edge session with user_id header verified), allow p_user_id
            v_target_id := p_user_id;
        END IF;
    ELSE
    ...
    IF v_token IS NULL OR p_force_rotate THEN
        -- 256-Bit cryptographic hex token (64 characters)
...
GRANT EXECUTE ON FUNCTION public.get_or_rotate_calendar_token(UUID, BOOLEAN) TO authenticated, anon, service_role;
