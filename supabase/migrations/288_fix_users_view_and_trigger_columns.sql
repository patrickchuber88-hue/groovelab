-- Migration 288: Fix public.users view columns and handle_users_view_dml trigger
-- Ensures push notification channels (push_notif_weekly_digest, push_notif_chat, push_notif_practice_reminder)
-- and parent controls exist on public.users_raw, public.users (VIEW) and its trigger.

-- 1. Ensure all columns exist on users_raw, students, pending_students
...
-- 5. Attach INSTEAD OF trigger to public.users view
...
-- 6. Grant full DML permissions on view
