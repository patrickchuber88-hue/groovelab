-- Migration: 359_student_schedule_absence_rpc_and_schema_alignment.sql
-- Description: Authoritative, tenant-isolated RPCs for student schedule absences and undo-absences
-- Aligns schema: ensures teacher_acknowledged, canceled_by_role, school_id columns exist and schedule_status enum includes canceled_by_student

-- 1. Schema alignment (idempotent)
DO $$
BEGIN
  BEGIN
    ALTER TYPE schedule_status ADD VALUE IF NOT EXISTS 'canceled_by_student';
...
ALTER TABLE public.schedule_occurrences 
  ADD COLUMN IF NOT EXISTS teacher_acknowledged BOOLEAN NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS canceled_by_role TEXT,
  ADD COLUMN IF NOT EXISTS school_id UUID REFERENCES public.schools(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS idx_sched_occ_student_date_status
  ON public.schedule_occurrences(student_id, date, status);

-- 2. Authoritative RPC: cancel_student_schedule_occurrence
...
BEGIN
  ...
  -- 1. Validate student exists and resolve school & teacher
  ...
  -- 2. Check if p_occurrence_id is a valid UUID
  ...
  IF v_teacher_id IS NOT NULL THEN
    BEGIN
      INSERT INTO public.system_alerts (
        ...
        v_teacher_id,
        'Termin abgesagt',
        '❌ Absage durch Schüler: ' || v_student_name || ' hat den Termin am ' || v_date_str || ' um ' || v_time_str || ' Uhr abgesagt. Eltern wurden benachrichtigt.'
      );
    EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
      INSERT INTO public.campus_direct_messages (
        ...
        p_student_id,
        v_teacher_id,
        'Dein Unterrichtstermin am ' || v_date_str || ' um ' || v_time_str || ' Uhr fällt aus.',
...
-- 3. Authoritative RPC: undo_cancel_student_schedule_occurrence
  ...
  IF v_teacher_id IS NOT NULL THEN
    BEGIN
      INSERT INTO public.system_alerts (
        ...
        '✅ Reaktiviert: ' || v_student_name || ' hat den Termin am ' || v_date_str || ' um ' || v_time_str || ' Uhr wieder reaktiviert.'
      );
    EXCEPTION WHEN OTHERS THEN
    END;
    ...
    BEGIN
      INSERT INTO public.campus_direct_messages (
        ...
        p_student_id,
        v_teacher_id,
        'Der Ausfall für diesen Termin wurde zurückgenommen. Der Termin findet regulär statt:\n' || v_date_str || ' um ' || v_time_str || ' Uhr.',
        COALESCE(v_occ_id::text, p_occurrence_id),
        true,
        'reschedule_notification'
      );
