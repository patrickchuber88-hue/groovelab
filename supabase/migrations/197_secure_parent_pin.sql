-- 1. Drop trigger first to avoid dependency issues
DROP TRIGGER IF EXISTS trg_users_view_dml ON public.users;
DROP VIEW IF EXISTS public.users CASCADE;

-- 2. Recreate view with parent_pin redacted (returning NULL) and has_parent_pin status
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        UPDATE public.users_raw SET
            ...
            parent_pin = COALESCE(NEW.parent_pin, parent_pin) -- allow partial view updates without resetting
        WHERE id = OLD.id;
...
-- 4. Recreate trigger on the view
...
-- 5. Create secure server-side RPC verify function
CREATE OR REPLACE FUNCTION public.verify_parent_pin(student_id UUID, input_pin TEXT)
RETURNS BOOLEAN AS $$
DECLARE
    stored_pin TEXT;
BEGIN
    SELECT parent_pin INTO stored_pin FROM public.users_raw WHERE id = student_id;
    RETURN (stored_pin = input_pin) OR (stored_pin IS NULL AND input_pin = '0000') OR (stored_pin = '' AND input_pin = '0000');
END;
