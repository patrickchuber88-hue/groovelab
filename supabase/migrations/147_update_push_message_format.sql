-- Migration 147: Update push notification message format for reschedule proposed by teacher
-- Erstellt: 2026-06-10
  ...
  IF (TG_OP = 'UPDATE' AND (
    ...
    IF TG_OP = 'UPDATE' AND NEW.status = 'approved' AND (OLD.time_slot IS DISTINCT FROM NEW.time_slot OR OLD.day_of_week IS DISTINCT FROM NEW.day_of_week) THEN
      ...
      IF v_user_id IS NOT NULL AND v_is_campus_active = TRUE THEN
        ...
        v_body := 'Dein Termin wurde verschoben: ' || 
          CASE OLD.day_of_week
            WHEN 1 THEN 'Mo.'
            WHEN 2 THEN 'Di.'
            WHEN 3 THEN 'Mi.'
            WHEN 4 THEN 'Do.'
            ...
            WHEN 7 THEN 'So.'
            ELSE ''
          END || ' ' || substring(OLD.time_slot from 1 for 5) || ' Uhr -> ' ||
          CASE NEW.day_of_week
            ...
            WHEN 7 THEN 'So.'
            ELSE ''
          END || ' ' || substring(NEW.time_slot from 1 for 5) || ' Uhr. Bitte bestätige den neuen Termin.';
        v_url := '/';
