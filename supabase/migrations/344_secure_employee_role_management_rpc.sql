-- ==============================================================================
-- Migration 344: Secure Employee Role Management RPC & Hardened View DML
-- Enables School Admins & Secretaries to manage staff roles securely
-- Standards: OWASP ASVS Level 3 / Zero-Trust Defense-in-Depth / Fail-Closed
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- 1. AUTHORITATIVE RPC: update_employee_roles
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 6. Apply Atomic Update to users_raw
    ...
    BEGIN
        INSERT INTO public.audit_logs (
            school_id, user_id, action, target_type, target_id, details, created_at
        ) VALUES (
            v_target.school_id,
    ...
    EXCEPTION WHEN OTHERS THEN
        -- Suppress audit log table failure if table doesn't exist
    END;
...
-- ------------------------------------------------------------------------------
-- 2. HARDEN HANDLE_USERS_VIEW_DML: Allow School Staff to update roles safely
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        -- Check if caller is school staff in the same school
        BEGIN
            v_caller_role := public.get_current_user_role();
            v_caller_school_id := public.get_current_user_school_id();
            v_is_school_staff := (v_caller_role IN ('admin', 'secretary') AND v_caller_school_id = OLD.school_id);
        EXCEPTION WHEN OTHERS THEN
            v_is_school_staff := FALSE;
        END;

        -- ZERO-TRUST PRIVILEGE-ESCALATION GUARDS:
        -- Non-master admins cannot alter school_id or is_master_admin
        IF NOT v_is_master THEN
            NEW.school_id := OLD.school_id;
            NEW.is_master_admin := OLD.is_master_admin;

            -- If caller is NOT school staff, prevent mutating roles or unassigned role escalation
            IF NOT v_is_school_staff THEN
                NEW.roles := OLD.roles;
