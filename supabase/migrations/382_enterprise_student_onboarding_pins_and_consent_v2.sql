-- ==============================================================================
-- Migration 382: Enterprise Student Onboarding Pins & Consent v2
-- Standards: OWASP ASVS Level 3 / DSGVO Art. 7, 8 / § 8a SGB VIII / § 10 BDSG
--
-- 1. COLUMNS: Add parent_name, parental_consent_given_at, consent_version, campus_usage_mode
-- 2. RECREATE public.users VIEW & DML TRIGGER with the new consent columns
-- 3. RPC: public.complete_student_onboarding_v2()
--    - 2-Stage PIN hierarchy: 6-digit Parent PIN + 4-digit Student PIN
--    - Cryptographic SHA-256 hashing to private_auth.user_secrets
--    - Strict voluntary parental Opt-In for 1:1 chat & audio
--    - Revisionssicherer Audit-Trail in public.audit_logs
-- ==============================================================================

-- ------------------------------------------------------------------------------
-- 1. Ensure Columns in users_raw & Legacy Tables
-- ------------------------------------------------------------------------------
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS parent_name TEXT;
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS parental_consent_given_at TIMESTAMPTZ;
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS consent_version TEXT;
ALTER TABLE public.users_raw ADD COLUMN IF NOT EXISTS campus_usage_mode TEXT;

DO $$ 
BEGIN 
    BEGIN 
        ALTER TABLE public.students ADD COLUMN IF NOT EXISTS parent_name TEXT; 
        ALTER TABLE public.students ADD COLUMN IF NOT EXISTS parental_consent_given_at TIMESTAMPTZ;
        ALTER TABLE public.students ADD COLUMN IF NOT EXISTS consent_version TEXT;
        ALTER TABLE public.students ADD COLUMN IF NOT EXISTS campus_usage_mode TEXT;
    EXCEPTION WHEN OTHERS THEN NULL; 
    END;
    BEGIN 
        ALTER TABLE public.pending_students ADD COLUMN IF NOT EXISTS parent_name TEXT; 
        ALTER TABLE public.pending_students ADD COLUMN IF NOT EXISTS parental_consent_given_at TIMESTAMPTZ;
        ALTER TABLE public.pending_students ADD COLUMN IF NOT EXISTS consent_version TEXT;
        ALTER TABLE public.pending_students ADD COLUMN IF NOT EXISTS campus_usage_mode TEXT;
    EXCEPTION WHEN OTHERS THEN NULL; 
    END;
END $$;

-- ------------------------------------------------------------------------------
-- 2. RECREATE public.users VIEW WITH SECURITY BARRIER & ALL AUDIT/CONSENT FIELDS
-- ------------------------------------------------------------------------------
...
-- ------------------------------------------------------------------------------
-- 3. AUTHORITATIVE RPC: complete_student_onboarding_v2
-- ------------------------------------------------------------------------------
...
BEGIN
    ...
    -- 9. Immutable Audit Log Trail (GoBD / Art. 8 DSGVO)
    BEGIN
        INSERT INTO public.audit_logs (
            ...
            NULL,
            jsonb_build_object(
                'parent_name', v_clean_parent_name,
                'consent_version', 'v2.0',
                'campus_usage_mode', v_clean_mode,
                'chat_opt_in', v_allow_chat,
                'student_audio_opt_in', v_allow_student_audio,
                'teacher_audio_opt_in', v_allow_teacher_audio,
    ...
    RETURN jsonb_build_object(
        'success', true,
        'user_id', v_user_id,
        'message', 'Onboarding und elterliche Freigabe erfolgreich abgeschlossen.'
    );
END;
$$;

GRANT EXECUTE ON FUNCTION public.complete_student_onboarding_v2(TEXT, TEXT, TEXT, TEXT, TEXT, JSONB) TO anon, authenticated, service_role;
