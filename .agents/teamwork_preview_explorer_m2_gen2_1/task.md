# Explorer Task: M2 Database Migration Remediate - Instance 1

## Objective
Analyze the database requirements for Groovelab Event Coordinator Overhaul and redesign the database migration file `supabase/migrations/173_event_coordinator_schema.sql` to resolve the integrity violations, security leaks, and bulk insert failures identified by the Forensic Auditor.

## Forensic Auditor Full Evidence Report
The milestone M2 failed due to the following audit report:
...
## Scope & Instructions for Explorer
1. Analyze the database requirements and the Auditor's findings.
2. Redesign the SQL schema migration script `supabase/migrations/173_event_coordinator_schema.sql` to resolve all issues:
   - **No backdoors**: Do not use the `x-bypass-forcing` header check or any other bypass check.
   - **Fix PostgREST Bulk Insert Failures**: For columns with `NOT NULL` constraints (like `is_pause`, `chairs_needed`, `music_stands_needed`, `performer_count`, `stage_number`, `sort_order`, `status`), replace incoming `NULL` values with their default values in the trigger `BEFORE INSERT OR UPDATE` using `COALESCE` statements so that bulk inserts do not fail with not-null constraint errors.
   - **Fix Database RLS Leak**: Write a correct `campus_events_select` RLS policy that replaces/fixes the existing leaky one on the database, properly checking `public.get_current_user_role()`.
   - **Trigger constraints**: Make sure trigger functions enforce business rules authentically for all user roles.
3. Write your redesigned database migration findings and recommended SQL schema in `handoff.md` within this directory.
