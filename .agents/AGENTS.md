...
- **Revisionssichere UI-Level-Governance & Cross-Origin-Echtzeitsynchronisation**: Wird der didaktische UI-Level (`campus_ui_level`: `junior`, `teen`, `pro`) im Elternbereich (z. B. in der PWA auf dem Smartphone) geändert, MUSS diese Änderung autoritativ und revisionssicher in der Datenbank (`save_parent_controls` / `public.audit_logs`) gespeichert werden. Gleichzeitig MUSS die Änderung in Echtzeit via Supabase Realtime Broadcast auf alle aktiven Instanzen (Localhost, Online-Web, Schülertablets) ohne Seiten-Reload übertragen werden. Die Datenbank (`users.campus_ui_level`) fungiert ausnahmslos als Single Source of Truth (SSOT) und überschreibt clientseitige Caches (`localStorage`) beim Starten und bei Hintergrundabfragen.
