...
import { Music, AlertCircle, Library, Shield, LogOut, Users, User, Monitor, QrCode, Plus, Pencil, Trash2, Box, BarChart as LucideBarChart, Clock, Star, PieChart as LucidePieChart, TrendingUp, Tablet, ExternalLink, Settings, Search, Bell, MapPin, X, Printer, Award } from 'lucide-react';
import { 
  ResponsiveContainer,
...
export function AdminDashboard({ userId, onLogout, forceTab, onTabChange, onOpenBandProfile }: AdminDashboardProps) {
  ...
  const handleAddMember = async (bandId: string, userId: string, instrument: string) => {
    try {
  ...
  const handleAddStudent = async (e: React.FormEvent) => {
    ...
    if (error) alert('Fehler: ' + error.message);
      ...
      setNewStudent({ firstName: '', lastName: '', birthDate: '', photoUrl: '' }); 
    }
  };
      ...
      <div style={{ marginTop: '24px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="flex-between" style={{ alignItems: 'flex-end' }}>
            <div>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '12px', margin: '0 0 8px 0' }}>
                <Award size={24} color={brandColor} /> Band-Verwaltung
              </h2>
              <p style={{ color: '#64748b', margin: 0, fontWeight: 600 }}>Alle Bands der Akademie ({filteredBands.length})</p>
            </div>
            ...
            <div style={{ position: 'relative', width: '300px' }}>
              ...
              <input 
                ...
                style={{ width: '100%', padding: '12px 16px 12px 48px', borderRadius: '16px', border: '2px solid #f1f5f9', background: 'white', fontWeight: 600, outline: 'none', transition: 'all 0.2s' }}
                  ...
                  {(editingBand.band_members || []).map((m: any) => {
                    ...
                    return (
                      <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc', padding: '12px 16px', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                          <img src={u?.photo_url || '/avatar_ghost.jpg'} style={{ width: '32px', height: '32px', borderRadius: '10px', objectFit: 'cover' }} />
                          <div>
                            <div style={{ fontWeight: 800, fontSize: '0.9rem', color: '#1e293b' }}>{u?.first_name} {u?.last_name}</div>
              ...
              {(() => {
                ...
                <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                   ...
                   >
                     ...
                     {Object.keys(INSTRUMENT_ICONS).map(inst => <option key={inst} value={inst}>{inst}</option>)}
                   </select>
                ));
              })()}
            </div>
