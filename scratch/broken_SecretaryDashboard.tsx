...
export function SecretaryDashboard({ schoolId, userId, onLogout }: SecretaryDashboardProps) {
  ...
  const [secretarySubTab, setSecretarySubTab] = useState<'briefing' | 'employees' | 'linking' | 'licenses' | 'setup' | 'profile'>('briefing');
  ...
  const [profileFormState, setProfileFormState] = useState<any>(null);
  ...
  // Redesigned Employee Onboarding & Search states
  const [employeeSearchQuery, setEmployeeSearchQuery] = useState<string>('');
  const [employeeAlphabetLetter, setEmployeeAlphabetLetter] = useState<string>('All');
  const [employeeStatusTab, setEmployeeStatusTab] = useState<'all' | 'admin' | 'secretary'>('all');
  const [showAddEmployeeModal, setShowAddEmployeeModal] = useState<boolean>(false);
  const [showEmployeeCsvImportModal, setShowEmployeeCsvImportModal] = useState<boolean>(false);
  ...
  const runAutoRoomAllocation = () => {
    ...
    for (const plan of sortedPlans) {
      ...
      if (instr.includes('schlagzeug') || instr.includes('drums')) {
        candidates = rooms.filter(r => r.name.toLowerCase().includes('schlagzeug') || r.name.toLowerCase().includes('drums') || r.name.toLowerCase().includes('band') || r.name.toLowerCase().includes('drum'));
        if (candidates.length === 0) candidates = [...rooms];
      } else if (instr.includes('klavier') || instr.includes('piano')) {
        candidates = rooms.filter(r => r.name.toLowerCase().includes('klavier') || r.name.toLowerCase().includes('piano') || r.name.toLowerCase().includes('flügel'));
        if (candidates.length === 0) candidates = [...rooms];
      }
  ...
  const handleDropOnMatrix = (targetRoomId: string | null, dayOfWeek: number) => {
        ...
        <div style={{ borderTop: activeTab === 'campus' ? '1px solid #d1fae5' : '1px solid #fef3c7', paddingTop: '20px' }}>
          <div 
            ...
            style={{ 
              ...
              cursor: 'pointer',
              transition: 'all 0.2s ease',
              backgroundColor: secretarySubTab === 'profile' ? '#fffbeb' : 'transparent',
              marginBottom: '8px'
            }}
          >
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <button 
              ...
              style={{ 
                ...
                color: activeTab === 'groovelab' ? '#09090b' : '#ffffff',
                width: '42px',
                height: '42px',
                padding: 0,
                display: 'flex',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ color: activeTab === 'campus' ? '#64748b' : '#a1a1aa', fontWeight: 500, fontSize: '0.85rem', marginTop: '4px', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                {currentUserProfile 
                  ? `${currentUserProfile.first_name} ${currentUserProfile.last_name || ''} • Schulverwaltung` 
                  : 'Schulverwaltung'
                }
              </p>
            </div>
        ...
        {activeTab === 'secretary' && secretarySubTab === 'employees' && (() => {
          const filteredEmployees = employees.filter((emp: any) => {
            ...
            const matchesAlphabet = employeeAlphabetLetter === 'All' || 
              lName.toUpperCase().startsWith(employeeAlphabetLetter);
              
            const matchesRole = employeeStatusTab === 'all' ||
              (employeeStatusTab === 'admin' && emp.role === 'admin') ||
              (employeeStatusTab === 'secretary' && emp.role === 'secretary');
              
            return matchesSearch && matchesAlphabet && matchesRole;
          });

          const adminCount = employees.filter(emp => emp.role === 'admin').length;
          const secretaryCount = employees.filter(emp => emp.role === 'secretary').length;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '28px', width: '100%' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                <div style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%)', border: '1px solid #bfdbfe', padding: '24px', borderRadius: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
                  <span style={{ fontSize: '0.75rem', color: '#1e40af', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Gesamt Mitarbeiter</span>
                  <strong style={{ display: 'block', fontSize: '2.2rem', color: '#1e3a8a', marginTop: '6px', fontWeight: 900, fontFamily: 'Urbanist' }}>{employees.length}</strong>
                </div>
                <div style={{ background: 'linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%)', border: '1px solid #bbf7d0', padding: '24px', borderRadius: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
                  <span style={{ fontSize: '0.75rem', color: '#166534', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Verwaltung</span>
                  <strong style={{ display: 'block', fontSize: '2.2rem', color: '#14532d', marginTop: '6px', fontWeight: 900, fontFamily: 'Urbanist' }}>{secretaryCount}</strong>
                </div>
                <div style={{ background: 'linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%)', border: '1px solid #fde68a', padding: '24px', borderRadius: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
                  <span style={{ fontSize: '0.75rem', color: '#854d0e', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Administratoren</span>
                  <strong style={{ display: 'block', fontSize: '2.2rem', color: '#713f12', marginTop: '6px', fontWeight: 900, fontFamily: 'Urbanist' }}>{adminCount}</strong>
                </div>
              </div>
              ...
              <div className="google-card" style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 850, color: '#0f172a' }}>Mitarbeiter Onboarding via CSV</h3>
                      <p style={{ margin: '2px 0 0 0', fontSize: '0.78rem', color: '#64748b' }}>Mehrere Mitarbeiter gleichzeitig per Textliste importieren</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowEmployeeCsvImportModal(!showEmployeeCsvImportModal)}
                    className="google-btn-secondary"
                    style={{ fontSize: '0.8rem', padding: '8px 16px', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}
                  >
                    {showEmployeeCsvImportModal ? 'Ausblenden' : 'Importbereich öffnen'}
                  </button>
                </div>

                {showEmployeeCsvImportModal && (
                  <div style={{ marginTop: '20px', borderTop: '1px solid #f1f5f9', paddingTop: '20px' }}>
                    <p style={{ margin: '0 0 12px 0', fontSize: '0.8rem', color: '#475569', fontWeight: 600 }}>Formatvorlage kopieren oder Mitarbeiter eintragen:</p>
                    <p style={{ margin: '0 0 16px 0', fontSize: '0.75rem', color: '#64748b', fontFamily: 'monospace', background: '#f8fafc', padding: '8px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>Vorname; Nachname; E-Mail; Spitzname; Rolle (admin/secretary)</p>
                    <textarea
                      placeholder="Sabine; Meier; sabine@schule.de; Sabi; secretary&#10;Thomas; Schmidt; thomas@schule.de;; admin"
                    ...
                    <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                      ...
                      <button
                        onClick={() => setEmployeeCsvText('Sabine; Meier; sabine@schule.de; Sabi; secretary\nThomas; Schmidt; thomas@schule.de;; admin')}
                        className="google-btn-secondary"
              ...
              <div className="google-card" style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', background: '#f1f5f9', padding: '4px', borderRadius: '16px', gap: '4px' }}>
                    <button
                      onClick={() => setEmployeeStatusTab('all')}
                      style={{
                        padding: '8px 16px',
                        borderRadius: '12px',
                        border: 'none',
                        fontSize: '0.8rem',
                        fontWeight: 700,
                        background: employeeStatusTab === 'all' ? '#ffffff' : 'transparent',
                        color: employeeStatusTab === 'all' ? '#0f172a' : '#64748b',
                        boxShadow: employeeStatusTab === 'all' ? '0 4px 6px -1px rgba(0,0,0,0.05)' : 'none',
                        cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                    >
                    ...
                    <button
                      onClick={() => setEmployeeStatusTab('secretary')}
                      style={{
                        padding: '8px 16px',
                        borderRadius: '12px',
                        border: 'none',
                        fontSize: '0.8rem',
                        fontWeight: 700,
                        background: employeeStatusTab === 'secretary' ? '#ffffff' : 'transparent',
                        color: employeeStatusTab === 'secretary' ? '#166534' : '#64748b',
                        boxShadow: employeeStatusTab === 'secretary' ? '0 4px 6px -1px rgba(0,0,0,0.05)' : 'none',
                        cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                    >
                    ...
                    <button
                      onClick={() => setEmployeeStatusTab('admin')}
                      style={{
                        padding: '8px 16px',
                        borderRadius: '12px',
                        border: 'none',
                        fontSize: '0.8rem',
                        fontWeight: 700,
                        background: employeeStatusTab === 'admin' ? '#ffffff' : 'transparent',
                        color: employeeStatusTab === 'admin' ? '#854d0e' : '#64748b',
                        boxShadow: employeeStatusTab === 'admin' ? '0 4px 6px -1px rgba(0,0,0,0.05)' : 'none',
                        cursor: 'pointer',
                        transition: 'all 0.2s'
                      }}
                    >
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', borderTop: '1px solid #f1f5f9', paddingTop: '16px' }}>
                  {['All', ...'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')].map(letter => {
                    const isSelected = employeeAlphabetLetter === letter;
                    return (
                      <button
                        key={letter}
                        onClick={() => setEmployeeAlphabetLetter(letter)}
                        style={{
              ...
              {/* DYNAMIC EMPLOYEES GRID */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '0.88rem', color: '#64748b', fontWeight: 600 }}>
                    Suchtreffer: {filteredEmployees.length} {filteredEmployees.length === 1 ? 'Mitarbeiter' : 'Mitarbeiter'}
                  </span>
                  {(employeeSearchQuery || employeeAlphabetLetter !== 'All' || employeeStatusTab !== 'all') && (
                    <button
                      onClick={() => {
                        setEmployeeSearchQuery('');
                        setEmployeeAlphabetLetter('All');
                        setEmployeeStatusTab('all');
                      }}
                ...
                {filteredEmployees.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '48px 24px', borderRadius: '24px', background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                    ...
                    <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, color: '#475569' }}>Keine Mitarbeiter gefunden</h3>
                    <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: '#94a3b8' }}>Passe deine Filterkriterien an oder lege einen neuen Mitarbeiter an.</p>
                  </div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
                    {filteredEmployees.map((emp: any) => {
                      const hash = (emp.last_name || '').charCodeAt(0) || 65;
                      ...
                      return (
                        <div
                          ...
                          onClick={() => setManageTeacher({
                            ...
                            email: emp.email || '',
                            instrument: '',
                            ausweisNummer: emp.ausweis_nummer || '',
                            isCampusActive: emp.is_campus_active ?? false,
                            isGroovelabActive: emp.is_groovelab_active ?? false,
                            isActive: emp.is_active ?? true,
                            role: emp.role || 'secretary',
                            nickname: emp.nickname || '',
                            teacherQrToken: emp.teacher_qr_token || emp.teacherQrToken || ''
                          })}
                          style={{
                            padding: '24px',
                            borderRadius: '24px',
                          ...
                          onMouseEnter={(e) => {
                            e.currentTarget.style.borderColor = '#0b57d0';
                            e.currentTarget.style.boxShadow = '0 12px 24px -10px rgba(0,0,0,0.06)';
                          }}
                          onMouseLeave={(e) => {
                            e.currentTarget.style.borderColor = '#e2e8f0';
                        ...
                        >
                          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                            <div style={{
                              ...
                              {(emp.first_name || 'M')?.[0]}{(emp.last_name || 'A')?.[0]}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <h4 style={{ margin: 0, fontSize: '0.92rem', fontWeight: 800, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {emp.first_name} {emp.last_name}
                              </h4>
                              <p style={{ margin: '2px 0 0 0', fontSize: '0.72rem', color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                {emp.email}
                              </p>
                            </div>
                          </div>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '4px' }}>
                            <span style={{ 
                              ...
                              padding: '4px 10px', 
                              borderRadius: '100px', 
                              background: emp.role === 'admin' ? '#fef3c7' : '#e6f4ea',
                              color: emp.role === 'admin' ? '#b45309' : '#137333'
                            }}>
                              {emp.role === 'admin' ? 'admin' : 'verwaltung'}
                            </span>
                            {emp.nickname && (
                              <span style={{ fontSize: '0.62rem', fontWeight: 800, textTransform: 'uppercase', padding: '4px 10px', borderRadius: '100px', background: '#f1f5f9', color: '#475569' }}>
                                "{emp.nickname}"
                              </span>
                            )}
                          </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #f1f5f9', paddingTop: '12px', marginTop: '4px' }}>
                            <div>
                              <span style={{ fontSize: '0.58rem', color: '#94a3b8', textTransform: 'uppercase', fontWeight: 800, display: 'block' }}>Mitarbeiter-PIN</span>
                              <strong style={{ fontSize: '0.8rem', fontFamily: 'monospace', color: '#475569' }}>
                                {emp.ausweis_nummer || 'Keine'}
                              </strong>
                            </div>
                          </div>
                        </div>
              ...
              {showAddEmployeeModal && (
                <div style={{
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#0f172a', fontFamily: 'Urbanist' }}>👤 Neuen Mitarbeiter anlegen</h3>
                      <button 
                    ...
                    <form onSubmit={(e) => {
                      ...
                      <div style={{ display: 'flex', gap: '12px' }}>
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          ...
                          <input required type="text" value={employeeFirstName} onChange={e => setEmployeeFirstName(e.target.value)} style={{ padding: '10px 14px', borderRadius: '12px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.85rem' }} />
                        </div>
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <label style={{ fontSize: '0.72rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase' }}>Nachname *</label>
                          <input required type="text" value={employeeLastName} onChange={e => setEmployeeLastName(e.target.value)} style={{ padding: '10px 14px', borderRadius: '12px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.85rem' }} />
                        </div>
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <label style={{ fontSize: '0.72rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase' }}>Spitzname</label>
                        <input type="text" value={employeeNickname} onChange={e => setEmployeeNickname(e.target.value)} style={{ padding: '10px 14px', borderRadius: '12px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.85rem' }} />
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        <label style={{ fontSize: '0.72rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase' }}>E-Mail-Adresse *</label>
                        <input required type="email" value={employeeEmail} onChange={e => setEmployeeEmail(e.target.value)} style={{ padding: '10px 14px', borderRadius: '12px', border: '1px solid #cbd5e1', outline: 'none', fontSize: '0.85rem' }} />
                      </div>

                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '12px' }}>
                        <button type="button" onClick={() => setShowAddEmployeeModal(false)} className="google-btn-secondary" style={{ flex: 1 }}>Abbrechen</button>
                        <button type="submit" className="google-btn-primary" style={{ flex: 1, background: '#0b57d0', color: '#ffffff' }}>Mitarbeiter anlegen</button>
                      </div>
                    </form>
                  </div>
                </div>
          ...
          <div style={{ display: 'flex', gap: '32px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1.6, display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              {campusSubTab === 'schedules' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', width: '105%', fontFamily: 'Inter, sans-serif' }}>
                  
                  {/* Action Toolbar */}
                  <div style={{ background: 'white', borderRadius: '24px', padding: '20px 24px', border: '1px solid rgba(0, 0, 0, 0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
                    <div>
                      <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 900, color: '#0f172a', fontFamily: 'Urbanist' }}>🗓️ Raumplanungs- & Freigabe-Zentrale</h3>
                      <p style={{ margin: '2px 0 0 0', fontSize: '0.78rem', color: '#64748b', fontWeight: 550 }}>Verteile Lehrkräfte auf Räume, erkenne Konflikte in Echtzeit oder nutze die 1-Klick-Zuteilung.</p>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      <button
                        ...
                        style={{ background: 'white', border: '1.5px solid #cbd5e1', color: '#475569', fontWeight: 800, padding: '10px 18px', borderRadius: '12px', fontSize: '0.78rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', opacity: matrixAllocations.filter(p => !p.roomId).length === 0 ? 0.55 : 1, transition: 'all 0.2s' }}
                      >
                      ...
                      <button
                        ...
                        style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', fontWeight: 800, padding: '11px 20px', borderRadius: '12px', fontSize: '0.78rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.15)' }}
                      >
                        💾 Raumzuteilung speichern & freigeben
                      </button>
                    </div>
                  </div>

                  {/* Main Planner Grid Workspace */}
                  <div style={{ display: 'grid', gridTemplateColumns: '240px minmax(0, 1fr)', gap: '20px', alignItems: 'start' }}>
                    
                    {/* Left Column Sidebar: Unassigned / Queue */}
                    <div style={{ background: 'white', borderRadius: '24px', border: '1px solid rgba(0, 0, 0, 0.05)', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px', minHeight: '450px' }}>
                      <div>
                        <h4 style={{ margin: 0, fontSize: '0.9rem', fontWeight: 800, color: '#0f172a' }}>Warteschlange</h4>
                        <span style={{ fontSize: '0.72rem', color: '#94a3b8', fontWeight: 600 }}>Ausstehende Raumzuteilungen ({matrixAllocations.filter(p => !p.roomId).length})</span>
                      </div>

                      <div 
                        onDragOver={(e) => e.preventDefault()}
                        onDrop={() => handleDropOnMatrix(null, 0)}
                        style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, minHeight: '350px', background: '#f8fafc', border: '2px dashed #e2e8f0', borderRadius: '16px', padding: '8px' }}
                      >
                        {matrixAllocations.filter(p => !p.roomId).map(plan => (
                          <div
                            key={plan.id}
                            draggable
                            onDragStart={() => handleDragStartMatrix(plan.id)}
                            onClick={() => setSelectedDayPlan(plan)}
                            style={{ background: 'white', border: '1px solid #e2e8f0', borderLeft: '4px solid #f59e0b', borderRadius: '12px', padding: '10px 12px', cursor: 'grab', display: 'flex', flexDirection: 'column', gap: '4px', boxShadow: '0 2px 4px rgba(0,0,0,0.01)' }}
                          >
                            <span style={{ fontSize: '0.76rem', fontWeight: 800, color: '#0f172a' }}>{plan.teacherName}</span>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.66rem', fontWeight: 700, color: '#94a3b8' }}>
                              <span>{plan.instrument}</span>
                              <span style={{ background: '#fffbeb', color: '#b45309', padding: '1px 4px', borderRadius: '4px' }}>
                                {['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'][plan.dayOfWeek]}
                              </span>
                            </div>
                            <span style={{ fontSize: '0.68rem', fontWeight: 900, fontFamily: 'monospace', color: '#f59e0b', marginTop: '2px' }}>
                              ⏱️ {plan.startTime} - {plan.endTime}
                            </span>
                          </div>
                        ))}

                        {matrixAllocations.filter(p => !p.roomId).length === 0 && (
                          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#94a3b8', padding: '24px', textAlign: 'center' }}>
                            <span style={{ fontSize: '1.5rem', marginBottom: '4px' }}>🎉</span>
                            <span style={{ fontSize: '0.72rem', fontWeight: 700 }}>Alle eingeteilt!</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div style={{ overflowX: 'auto', background: 'white', borderRadius: '24px', border: '1px solid rgba(0, 0, 0, 0.05)', padding: '20px' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid #f1f5f9', textAlign: 'left' }}>
                            <th style={{ width: '130px', padding: '12px 8px', fontSize: '0.75rem', fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Raum</th>
                            {[1, 2, 3, 4, 5].map(dayNum => (
                              <th key={dayNum} style={{ padding: '12px 8px', fontSize: '0.75rem', fontWeight: 900, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                {['', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag'][dayNum]}
                              </th>
                            ))}
                          </tr>
                        </thead>
                          ...
                          {rooms.map(room => (
                            <tr key={room.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                              <td style={{ padding: '14px 8px', verticalAlign: 'middle' }}>
                                <div style={{ display: 'flex', flexDirection: 'column' }}>
                                  <strong style={{ fontSize: '0.8rem', color: '#0f172a', fontWeight: 800 }}>{room.name}</strong>
                                  <span style={{ fontSize: '0.62rem', color: '#94a3b8', fontWeight: 700 }}>Kapazität: 1 Coach</span>
                                </div>
                              </td>
                              
                              {[1, 2, 3, 4, 5].map(dayNum => {
                                // Filter allocations for this room and day
                                ...
                                return (
                                  <td
                                    ...
                                    style={{ padding: '8px', verticalAlign: 'top', background: draggedPlanId ? '#fdfdfd' : 'transparent', transition: 'background 0.2s' }}
                                  >
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minHeight: '65px', justifyContent: 'center' }}>
                                      {cellPlans.map(plan => {
                                        // Check for overlap conflicts in this cell
                                        const hasOverlap = cellPlans.some(p => p.id !== plan.id && p.startTime < plan.endTime && plan.startTime < p.endTime);
                                        const borderStyle = hasOverlap ? '2px dashed #ef4444' : '1px solid #e2e8f0';
                                        const backgroundStyle = hasOverlap ? '#fef2f2' : 'white';
                                        ...
                                        return (
                                          <div
                                            ...
                                            onDragStart={() => handleDragStartMatrix(plan.id)}
                                            onClick={() => setSelectedDayPlan(plan)}
                                            style={{ background: backgroundStyle, border: borderStyle, borderLeft: hasOverlap ? '4px solid #ef4444' : '4px solid #10b981', borderRadius: '12px', padding: '8px 10px', cursor: 'grab', display: 'flex', flexDirection: 'column', gap: '3px', boxShadow: '0 2px 4px rgba(0,0,0,0.01)', position: 'relative' }}
                                          >
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                              <span style={{ fontSize: '0.74rem', fontWeight: 800, color: hasOverlap ? '#991b1b' : '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                {plan.teacherName}
                                              </span>
                                              {hasOverlap && (
                                                <span style={{ fontSize: '0.65rem', color: '#ef4444', fontWeight: 900 }} title="Zeitkonflikt / Überlappung!">⚠️</span>
                                              )}
                                            </div>
                                            ...
                                            <span style={{ fontSize: '0.65rem', fontWeight: 900, fontFamily: 'monospace', color: hasOverlap ? '#ef4444' : '#059669', marginTop: '2px' }}>
                                              ⏱️ {plan.startTime} - {plan.endTime}
                                            </span>
                                          </div>
                                        );
                  ...
                  {/* Right Sliding Detail Panel (Drawer Modal) */}
                  {selectedDayPlan && (
                    <div style={{ position: 'fixed', top: 0, right: 0, width: '420px', height: '100vh', background: 'white', boxShadow: '-10px 0 40px rgba(15,23,42,0.12)', borderLeft: '1px solid #e2e8f0', zIndex: 1050, display: 'flex', flexDirection: 'column', padding: '24px', animation: 'modalFadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid #f1f5f9', paddingBottom: '16px' }}>
                        <div>
                          <span style={{ fontSize: '0.65rem', fontWeight: 800, color: '#f59e0b', background: '#fffbeb', border: '1px solid rgba(245,158,11,0.2)', padding: '2px 8px', borderRadius: '6px', textTransform: 'uppercase' }}>
                            {['Sonntag', 'Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag'][selectedDayPlan.dayOfWeek]} Plan
                          </span>
                          <h3 style={{ margin: '6px 0 0 0', fontSize: '1.2rem', fontWeight: 900, color: '#0f172a', fontFamily: 'Urbanist' }}>
                            {selectedDayPlan.teacherName}
                          </h3>
                          <span style={{ fontSize: '0.72rem', color: '#94a3b8', fontWeight: 700, display: 'block', marginTop: '2px' }}>
                            🎸 Fach: {selectedDayPlan.instrument}
                          </span>
                        </div>
                        <button 
                      ...
                      {/* Room assignment & actions block */}
                      <div style={{ padding: '16px 0', display: 'flex', flexDirection: 'column', gap: '12px', borderBottom: '1px solid #f1f5f9' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                          <label style={{ fontSize: '0.68rem', fontWeight: 900, textTransform: 'uppercase', color: '#94a3b8' }}>Unterrichtsraum zuweisen</label>
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              setMatrixAllocations(prev => prev.map(p => {
                                if (p.id === selectedDayPlan.id) {
                                  ...
                                  // update selected plan to reflect new room in drawer as well
                            ...
                            style={{ background: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '10px', padding: '8px 10px', fontSize: '0.8rem', fontWeight: 700, color: '#475569', outline: 'none', cursor: 'pointer' }}
                          >
                        ...
                        <button
                          ...
                          style={{ background: 'rgba(239, 68, 68, 0.05)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)', padding: '10px 14px', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer', transition: 'all 0.2s', width: '100%', marginTop: '6px' }}
                        >
                          ↩ Zur Überarbeitung zurückweisen
                        </button>
                      </div>

                      {/* Day Timetable slots list */}
                      <div style={{ flex: 1, overflowY: 'auto', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        <h4 style={{ margin: '0 0 4px 0', fontSize: '0.75rem', fontWeight: 900, textTransform: 'uppercase', color: '#94a3b8' }}>Stundenliste</h4>
                        
                        {selectedDayPlan.slots.map((slot: any, idx: number) => {
                          const isBreak = !slot.student_id;
                          return (
                            <div key={idx} style={{ padding: '10px 12px', borderRadius: '12px', border: '1px solid #f1f5f9', background: isBreak ? '#fffbeb' : '#f8fafc', borderLeft: isBreak ? '4px solid #f59e0b' : '4px solid #3b82f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <div>
                                <span style={{ fontSize: '0.78rem', fontWeight: 800, color: '#1d1d1f' }}>
                                  {isBreak ? '☕ Pause' : slot.student_name}
                                </span>
                                <span style={{ fontSize: '0.62rem', color: '#94a3b8', fontWeight: 650, display: 'block', marginTop: '2px' }}>
                                  {isBreak ? 'Regenerations-Intervall' : `Schüler-Fach: ${selectedDayPlan.instrument}`}
                                </span>
                              </div>
                              <div style={{ textAlign: 'right' }}>
                                <span style={{ fontSize: '0.74rem', fontWeight: 900, fontFamily: 'monospace', color: isBreak ? '#b45309' : '#1d1d1f' }}>
                                  {slot.time_slot}
                                </span>
                                <span style={{ fontSize: '0.62rem', color: '#94a3b8', fontWeight: 700, display: 'block', marginTop: '1px' }}>
                                  {slot.duration} Min
                                </span>
                              </div>
                            </div>
              ...
              )}', color: '#ea4335', borderColor: '#fce8e6' }}>Ablehnen</button>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        {/* TAB 1.9: SECRETARY - PROFILE */}
        {activeTab === 'secretary' && secretarySubTab === 'profile' && (
          <div className="animation-slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '32px', maxWidth: '1000px' }}>
            <style>{`
              .profile-input {
                width: 100%;
                box-sizing: border-box;
            ...
            {(() => {
              if (!profileFormState && currentUserProfile) {
                setProfileFormState({
            ...
            <div className="google-card" style={{ padding: '36px', borderRadius: '32px', border: '1px solid #f1f5f9', background: 'white' }}>
              <div className="google-kpi-bar bg-google-red" />
              
              <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
                {/* Left Profile Avatar Panel */}
                <div style={{ flex: '1 1 280px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px', borderRight: '1px solid #f1f5f9', paddingRight: '40px' }}>
                  <div style={{ position: 'relative', width: '120px', height: '120px', borderRadius: '32px', overflow: 'hidden', border: '4px solid white', boxShadow: '0 12px 32px rgba(0,0,0,0.08)' }}>
                    {profileFormState?.photo_url ? (
                      <img src={profileFormState.photo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <div style={{ width: '100%', height: '100%', background: '#fee2e2', color: '#ea4335', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: '2.5rem' }}>
                        {profileFormState?.first_name?.[0] || 'S'}
                      </div>
                    )}
                  </div>
                  
                  <div style={{ textAlign: 'center' }}>
                    <h4 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 900, color: '#0f172a', fontFamily: 'Urbanist' }}>
                      {currentUserProfile?.first_name} {currentUserProfile?.last_name}
                    </h4>
                  ...
                  <div style={{ width: '100%' }}>
                    <label style={{ fontSize: '0.7rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', display: 'block', marginBottom: '8px', textAlign: 'center' }}>Schnell-Avatare</label>
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
                      {[
                        '/avatar_ghost.jpg',
                        '/avatars/band_avatar_new_01.png',
                        '/avatars/band_avatar_new_02.png',
                        '/avatars/band_avatar_new_03.png',
                        '/avatars/band_avatar_new_04.png'
                      ].map((url) => (
                        <div 
                          key={url}
                          className={`avatar-choice ${profileFormState?.photo_url === url ? 'active' : ''}`}
                          onClick={() => setProfileFormState({ ...profileFormState, photo_url: url })}
                        >
                ...
                <form 
                  onSubmit={async (e) => {
                    e.preventDefault();
                    if (!userId || !profileFormState) return;
                    setIsSavingProfile(true);
                    try {
                      const { error } = await supabase
                        .from('users')
                        .update({
                          first_name: profileFormState.first_name,
                          last_name: profileFormState.last_name,
                          nickname: profileFormState.nickname,
                          email: profileFormState.email,
                          phone: profileFormState.phone,
                          photo_url: profileFormState.photo_url
                        })
                        .eq('id', userId);
                      if (error) throw error;
                  ...
                  style={{ flex: '2 1 400px', display: 'flex', flexDirection: 'column', gap: '24px' }}
                >
                  <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: '#0f172a', fontFamily: 'Urbanist' }}>Mein Profil verwalten</h3>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      <input 
                        ...
                        onChange={(e) => setProfileFormState({ ...profileFormState, first_name: e.target.value })} 
                      />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      ...
                      <input 
                        ...
                        onChange={(e) => setProfileFormState({ ...profileFormState, last_name: e.target.value })} 
                      />
                    </div>
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Anzeigename / Nickname</label>
                    <input 
                      className="profile-input" 
                      value={profileFormState?.nickname || ''} 
                      onChange={(e) => setProfileFormState({ ...profileFormState, nickname: e.target.value })} 
                    />
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <label style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>E-Mail-Adresse</label>
                      <input 
                        type="email" 
                        required 
                        className="profile-input" 
                        value={profileFormState?.email || ''} 
                        onChange={(e) => setProfileFormState({ ...profileFormState, email: e.target.value })} 
                      />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <label style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Telefonnummer</label>
                      <input 
                        className="profile-input" 
                        value={profileFormState?.phone || ''} 
                        onChange={(e) => setProfileFormState({ ...profileFormState, phone: e.target.value })} 
                      />
                    </div>
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <label style={{ fontSize: '0.75rem', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Profilbild-Link (Photo URL)</label>
                    <input 
                      className="profile-input" 
                      placeholder="https://..."
                      value={profileFormState?.photo_url || ''} 
                      onChange={(e) => setProfileFormState({ ...profileFormState, photo_url: e.target.value })} 
                    />
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '16px', marginTop: '12px' }}>
                    <button 
                      ...
                      style={{ 
                        ...
                        fontSize: '1rem', 
                        cursor: 'pointer', 
                        boxShadow: '0 8px 24px rgba(234, 67, 53, 0.2)' 
                      }}
                    >
                    ...
                    <button 
                      type="button" 
                      onClick={() => setSecretarySubTab('briefing')} 
                      style={{ 
                        flex: 1, 
                        background: '#f1f5f9', 
                        color: '#64748b', 
        ...
        return (
          <div style={{
            ...
            <div style={{ padding: '24px', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              {manageTeacher.role === 'admin' || manageTeacher.role === 'secretary' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <label style={{ fontSize: '0.72rem', fontWeight: 800, color: '#475569', textTransform: 'uppercase' }}>Systemrolle</label>
                  <select 
                    value={manageTeacher.role} 
                    onChange={(e) => setManageTeacher({ ...manageTeacher, role: e.target.value })}
                    style={{
                      padding: '10px 14px',
                      borderRadius: '12px',
                      border: '1px solid #cbd5e1',
                ...
                return link ? (
                  <div style={{
                    ...
                    <div style={{ 
                      ...
                      <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#0f172a', marginTop: '4px', fontFamily: 'Urbanist, sans-serif' }}>
                        {manageTeacher.firstName} {manageTeacher.lastName}
                      </div>
                      <div style={{ fontSize: '0.72rem', color: '#64748b', fontWeight: 600, textTransform: 'lowercase', fontFamily: 'Inter, sans-serif' }}>
                        {(() => {
                          const r = (manageTeacher.role || '').toLowerCase();
                          if (r === 'admin' || r === 'secretary' || r === 'verwaltung') return 'verwaltung';
                          if (r === 'teacher' || r === 'lehrer') return 'lehrer';
                          if (r === 'student' || r === 'schüler') return 'schüler';
                          return 'lehrer';
                        })()}
                      </div>
                    </div>
