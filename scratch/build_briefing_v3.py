            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div style={{ background: '#ffffff', borderRadius: '24px', padding: '32px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)', position: 'relative', overflow: 'hidden' }}>
                <h2 style={{ fontSize: '2rem', fontWeight: 900, margin: '0 0 16px 0', color: '#1e293b', fontFamily: "'Urbanist', sans-serif" }}>
                  Hallo. <span style={{ color: '#0b57d0' }}>{avatar?.student_name?.split(' ')[0] || 'Schüler'} 👋</span>
                </h2>
...
import_match = re.search(r"import \{(.*?)\} from 'lucide-react';", new_content)
if import_match:
    current_imports = import_match.group(1)
    if "Check" not in current_imports:
        new_imports = "Check, Star, Award, Sparkles, " + current_imports
        new_content = new_content.replace(import_match.group(0), f"import {{ {new_imports} }} from 'lucide-react';")

with open(filename, "w") as f:
    f.write(new_content)

print("Briefing V3 UI updated cleanly.")
