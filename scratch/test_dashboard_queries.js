...
async function main() {
  console.log("Fetching student profile Silas Meier...");
  ...
  // Setup queries
  const studentQuery = supabase.from('users').select('*').eq('school_id', studentUser.school_id).eq('role', 'student');
  ...
  `).eq('school_id', studentUser.school_id).eq('is_groovelab_active', true);

  const queries = {
    rooms: supabase.from('rooms').select('*').eq('school_id', studentUser.school_id).eq('is_groovelab_active', true).order('sort_order', { ascending: true }),
    user_availability: supabase.from('user_availability').select('*'),
    sessions: supabase.from('sessions').select('*, users!inner(*), stations(*)').is('check_out_time', null).eq('users.school_id', studentUser.school_id),
    coaches: supabase.from('users').select('*').in('role', ['teacher', 'admin', 'Teacher', 'Admin', 'TEACHER', 'ADMIN']).eq('school_id', studentUser.school_id),
    user_song_skills: supabase.from('user_song_skills').select('*, users!user_id(*), songs(*)').eq('is_pending_approval', true),
    bands: supabase.from('bands').select('*, band_members(*, users(*)), coach:users!coach_id(id, first_name, last_name, photo_url), band_songs(*, songs(*), band_song_slots(*, profiles:users!user_id(id, first_name, photo_url, user_song_skills:user_song_skills!user_song_skills_user_id_fkey(id, song_id, instrument, progress_percent, is_pending_approval, is_stage_ready))))').eq('school_id', studentUser.school_id).order('name'),
    students: studentQuery.order('first_name'),
    help_requests: Promise.resolve({ data: null, error: null }),
    formingBands: supabase.from('bands').select('*, band_members(*, profiles:users(id, first_name, last_name, photo_url, created_at, birth_date)), songs(*), band_songs(*, songs(*), band_song_slots(*, profiles:users!user_id(id, first_name, last_name, photo_url, created_at, birth_date)))').eq('school_id', studentUser.school_id).in('status', ['forming', 'active']),
    wallSongs: wallSongsQuery,
    occupiedSlots: supabase.from('band_song_slots').select('user_id, band_songs(song_id)'),
    crisis: supabase.from('crisis_notifications').select('*, student:users!crisis_notifications_student_id_fkey(id, first_name, last_name)').eq('teacher_id', SILAS_ID).gte('slot_start_datetime', new Date().toISOString()).order('slot_start_datetime', { ascending: true })
  };
