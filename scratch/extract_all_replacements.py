                    ...
                    if entry.get("source") == "MODEL" and "tool_calls" in entry:
                        for tc in entry["tool_calls"]:
                            if tc["name"] in ["replace_file_content"]:
                                ...
                                if fpath not in all_replacements:
                                    all_replacements[fpath] = []
                                all_replacements[fpath].append({
...
with open("scratch/all_ui_replacements_all_files.json", "w") as f:
    json.dump(all_replacements, f, indent=2)

print(f"Extracted replacements across {len(all_replacements)} files.")
