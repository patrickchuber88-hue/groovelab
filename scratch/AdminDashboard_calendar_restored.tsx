...
export function AdminDashboard({ userId, onLogout, forceTab, onTabChange, onOpenBandProfile, activePlatform = 'groovelab' }: AdminDashboardProps) {
  ...
  // Auto-correct end time if it is less than or equal to start time
  useEffect(() => {
    if (bookingStartTime && bookingEndTime) {
      ...
      if (!isNaN(sh) && !isNaN(sm) && !isNaN(eh) && !isNaN(em)) {
        ...
        if (endMin <= startMin) {
          const newEndMin = startMin + 30;
          const newEh = Math.floor(newEndMin / 60);
          const newEm = newEndMin % 60;
          const newEndTimeStr = `${String(newEh).padStart(2, '0')}:${String(newEm).padStart(2, '0')}`;
          setBookingEndTime(newEndTimeStr);
        }
      }
    }
  ...
  const renderCampusRoomsTab = () => {
    ...
    const getFloorWeight = (f: string) => {
      ...
      if (ogMatch) return parseInt(ogMatch[1], 10);
      const ugMatch = f.match(/^(\d+)\.\s*UG$/i);
      if (ugMatch) return -parseInt(ugMatch[1], 10);
      return 9999;
    };

    const uniqueFloors = Array.from(new Set(
      rooms.map((room: any) => room.floor || localFloorMappings[room.id] || 'Allgemein')
    )).sort((a: string, b: string) => {
      const weightA = getFloorWeight(a);
      const weightB = getFloorWeight(b);
      if (weightA !== weightB) return weightA - weightB;
      return a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' });
    });

    const activeRooms = isDateFilterActive ? availableRooms : rooms;
    const roomsToRender = activeRooms.filter((room: any) => {
      if (selectedFloor === 'Alle') return true;
      const roomFloor = room.floor || localFloorMappings[room.id] || 'Allgemein';
...
"    // Merge overlapping/consecutive bookings for the "Meine Buchungen" sidebar list
...
"    const changeWeek = (weeks: number) => {
    ...
    const getWeekdayDate = (dayIndex: number) => {
      ...
      const targetDate = new Date(currentSelectedDate.setDate(diffToMon + dayIndex));
    ...
    const getBookingsForSlot = (dayIndex: number, hourStr: string) => {
      ...
      // 1. Manual bookings
      const manualForSlot = roomBookings.filter((b: any) => {
      ...
      const schedulesForSlot = schedules
        .filter((s: any) => {
          if (s.room_id !== targetRoomId)
              ...
              {isDateFilterActive && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#78350f', fontWeight: 750 }}>
                      Anzeige gefiltert für: <strong>{new Date(bookingDate).toLocaleDateString('de-DE', { weekday: 'long', year: 'numeric', month: '2-digit', day: '2-digit' })}</strong> ({bookingStartTime} - {bookingEndTime})
                    </span>
                  </div>
                  <button
                    onClick={() => {
...
"                {rooms.map((room) => {
                  ...
                  const isOccupiedByManual = campusBookings.some((b: any) => {
                    if (b.roomId !== room.id || b.date !== localDateStr) return false;
                    const [shStr, smStr] = b.startTime.split(':');
                    const [ehStr, emStr] = b.endTime.split(':');
                    const bStart = (parseInt(shStr) || 0) * 60 + (parseInt(smStr) || 0);
                    const bEnd = (parseInt(ehStr) || 0) * 60 + (parseInt(emStr) || 0);
                    return localMin >= bStart && localMin < bEnd;
                  });
                  ...
                  const isOccupiedBySchedule = schedules.some((s: any) => {
                    ...
                    return localMin >= sStart && localMin < sEnd;
                  });

                  const isCurrentlyOccupied = isOccupiedByManual || isOccupiedBySchedule;
...
<truncated 1469 bytes>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', height: showMyBookingsOnly ? '100%' : 'auto' }}>
