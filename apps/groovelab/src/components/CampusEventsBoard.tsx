...
export const getEffectiveAppDate = (): Date => {
  if (typeof window === 'undefined') return new Date();
  const simStr = localStorage.getItem('groovelab_simulated_date');
  return simStr ? new Date(simStr + 'T14:00:00') : new Date();
};

// 🏛️ Tier-1 Helper: Read initial L1 SWR cache synchronously for 0ms First Contentful Paint
const readInitialEventsCache = (sId?: string, uId?: string) => {
  if (typeof window === 'undefined') return null;
  try {
    const keys = [
      `cg_events_swr_${sId || 'global'}_${uId || 'anon'}`,
      `cg_events_swr_global_${uId || 'anon'}`
    ];
    for (const k of keys) {
...
export function CampusEventsBoard({
  ...
  // 🏛️ Tier-1 Core Data States with Synchronous L1-Memory Hydration (0ms FCP)
  const initialCache = useMemo(() => readInitialEventsCache(schoolId, userId), [schoolId, userId]);

  const [lessons, setLessons] = useState<LessonOccurrence[]>(() => initialCache?.lessons || []);
  const lessonsRef = useRef<LessonOccurrence[]>(lessons);
  lessonsRef.current = lessons;
  const [customEvents, setCustomEvents] = useState<CampusEvent[]>(() => initialCache?.customEvents || []);
  ...
  const [loadingEvents, setLoadingEvents] = useState<boolean>(() => !initialCache?.customEvents || initialCache.customEvents.length === 0);
  ...
  const [schoolRooms, setSchoolRooms] = useState<{id: string; name: string; floor?: string}[]>(() => initialCache?.schoolRooms || []);
  ...
  const [schoolAnnouncements, setSchoolAnnouncements] = useState<any[]>(() => initialCache?.schoolAnnouncements || []);
  ...
  // 🏛️ Tier-1 SWR Instant Paint: Sofortiger 0ms Paint gecachter Termine & Stundenplandaten
  useEffect(() => {
    if (!schoolId && !userId) return;
    const cacheKey = `cg_events_swr_${schoolId || 'global'}_${userId || 'anon'}`;
    try {
      const swrRaw = localStorage.getItem(cacheKey);
      if (swrRaw) {
        const cached = JSON.parse(swrRaw);
        if (cached && cached.timestamp && Date.now() - Number(cached.timestamp) < 7 * 24 * 3600 * 1000) {
          if (Array.isArray(cached.customEvents) && cached.customEvents.length > 0) {
            setCustomEvents(cached.customEvents);
            setLoadingEvents(false);
          }
          if (Array.isArray(cached.schoolRooms) && cached.schoolRooms.length > 0) {
            setSchoolRooms(cached.schoolRooms);
          }
          if (Array.isArray(cached.schoolAnnouncements) && cached.schoolAnnouncements.length > 0) {
            setSchoolAnnouncements(cached.schoolAnnouncements);
          }
          if (Array.isArray(cached.lessons) && cached.lessons.length > 0) {
            setLessons(cached.lessons);
            setLoadingLessons(false);
          }
          if (cached.calendarUrl) {
            setCalendarUrl(cached.calendarUrl);
          }
          if (Array.isArray(cached.studentEnsembleIds)) {
            setStudentEnsembleIds(cached.studentEnsembleIds);
          }
          if (Array.isArray(cached.studentProgramPoints)) {
            setStudentProgramPoints(cached.studentProgramPoints);
          }
        }
      }
    } catch (e) {
      console.warn('[CampusEventsBoard] SWR cache instant paint error:', e);
    }
  }, [schoolId, userId]);
  ...
  useEffect(() => {
    ...
    // Fallback: If SWR cache is empty, trigger auxiliary loaders only after a delay as safety net
    const cacheKey = `cg_events_swr_${schoolId || 'global'}_${userId || 'anon'}`;
    ...
    const handleSync = () => {
       ...
       debounceTimer = setTimeout(() => {
        // Zero-Wipe SWR: Revalidate in background without wiping UI or showing loading spinner
  ...
  const fetchLessons = async (options?: { silent?: boolean }) => {
    ...
    try {
      ...
      const tempD = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()));
      tempD.setUTCDate(tempD.getUTCDate() + 4 - (tempD.getUTCDay() || 7));
      const yearStart = new Date(Date.UTC(tempD.getUTCFullYear(), 0, 1));
      const currentWeekNum = Math.ceil((((tempD.getTime() - yearStart.getTime()) / 86400000) + 1) / 7);
      const currentWeekKey = `${currentMonthKey}-W${String(currentWeekNum).padStart(2, '0')}`;
      ...
      // 🏛️ Tier-1 Single-Flight Low-Latency Bootstrap (OWASP ASVS Level 3 / <25ms Engine)
      ...
      if (effectiveSchoolId) {
        try {
          ...
          if (!bootErr && bootRes && typeof bootRes === 'object') {
            bootstrapData = bootRes;
          }
        } catch (e) {
          console.warn('[CampusEventsBoard] Single-flight bootstrap error, falling back:', e);
        }
      }
      ...
      let allSchoolRooms: any[] = [];
      const roomMap = new Map<string, any>();
      const userMap = new Map<string, any>();

      if (bootstrapData) {
        // 🏛️ Fast-Path: Single-flight populated in <25ms
        schedules = bootstrapData.schedules || [];
        occurrences = bootstrapData.occurrences || [];
        studentTeacherObj = bootstrapData.student_teacher || null;
        teacherProfileObj = bootstrapData.teacher_profile || null;
        allSchoolRooms = bootstrapData.rooms || [];
        if (teacherProfileObj) {
          setCurrentTeacherProfile(teacherProfileObj);
        }
        if (Array.isArray(bootstrapData.events)) {
          setCustomEvents(bootstrapData.events);
          setLoadingEvents(false);
        }
        if (Array.isArray(bootstrapData.rooms)) {
          setSchoolRooms(bootstrapData.rooms);
        }
        if (Array.isArray(bootstrapData.announcements)) {
          setSchoolAnnouncements(bootstrapData.announcements);
        }
        if (bootstrapData.calendar_settings) {
          ...
          if (bootstrapData.calendar_settings.calendar_url) {
            ...
            setTimeout(() => {
              fetchSubscribedCalendar(bootstrapData.calendar_settings.calendar_url);
            }, 100);
          }
        }
        if (Array.isArray(bootstrapData.student_ensembles)) {
          setStudentEnsembleIds(bootstrapData.student_ensembles);
        }
        if (Array.isArray(bootstrapData.student_program_points)) {
          setStudentProgramPoints(bootstrapData.student_program_points);
        }
        ...
        const [rawSchedulesRes, occurrencesRes, roomsRes] = await Promise.all([
          ...
          occurrenceQuery,
          effectiveSchoolId 
            ? supabase.from('rooms').select('id, name, school_id').eq('school_id', effectiveSchoolId)
            : Promise.resolve({ data: [] } as any),
          effectiveSchoolId ? fetchCustomEvents() : Promise.resolve(),
          effectiveSchoolId ? fetchSchoolCalendarSettings() : Promise.resolve(),
          effectiveSchoolId ? fetchAnnouncements() : Promise.resolve(),
          role === 'student' ? fetchStudentEnsembles() : Promise.resolve(),
          role === 'student' ? fetchStudentProgramPoints() : Promise.resolve()
        ]);

        schedules = rawSchedulesRes.data || [];
        occurrences = occurrencesRes.data || [];
        allSchoolRooms = roomsRes.data || [];
        allSchoolRooms.forEach((r: any) => {
          if (r.id) roomMap.set(r.id, r);
          if (r.name) roomMap.set(r.name.toLowerCase().trim(), r);
        });

        // Wave 2: Consolidated Batch Fetch for all related users (students & teachers)
        const allUserIds = Array.from(new Set([
          ...schedules.map((s: any) => s.student_id),
        ...
        // Map schedules with user and room references
        schedules = schedules.map((s: any) => {
          const tObj = s.teacher || userMap.get(s.teacher_id) || studentTeacherObj || null;
          ...
          return {
            ...s,
            student: s.student || userMap.get(s.student_id) || null,
            teacher: tObj,
            rooms: rObj,
            room: rObj,
        ...
        // Map occurrences with user and room references
        occurrences = occurrences.map((o: any) => {
          const tObj = o.teacher || userMap.get(o.teacher_id) || studentTeacherObj || null;
          ...
          return {
            ...o,
            student: o.student || userMap.get(o.student_id) || null,
            teacher: tObj,
            room: rObj,
            rooms: rObj,
            room_name: resolvedRoomName,
      ...
      if (teacherPlannedBoards && teacherPlannedBoards.length > 0) {
        ...
        if (bootstrapData && Array.isArray(bootstrapData.students) && bootstrapData.students.length > 0) {
          allStudentsList = bootstrapData.students;
        } else if (userMap.size > 0) {
          allStudentsList = Array.from(userMap.values());
        } else {
          let studentQuery = supabase
            .from('users')
            .select('id, first_name, last_name, instrument');
      ...
      // 🏛️ Tier-1 Additive Zero-Wipe SWR Merge: Never truncate already-rendered months/weeks!
      setLessons(prev => {
        if (!prev || prev.length === 0) return allMergedOccurrences;
        const occMap = new Map<string, any>();
        prev.forEach((item: any) => {
          if (item?.id) occMap.set(item.id, item);
        });
        allMergedOccurrences.forEach((item: any) => {
          if (item?.id) occMap.set(item.id, item);
        });
        const merged = Array.from(occMap.values());
        merged.sort((a, b) => {
          if (a.date !== b.date) return a.date.localeCompare(b.date);
          return (a.start_time || '').localeCompare(b.start_time || '');
        });
  ...
  const handleCancelOccurrence = async (occ: any, skipPinCheck = false) => {
    ...
    try {
      ...
      const canceledByRole = role === 'student' ? 'student' : 'teacher';
...
<body>
                    ...
                    role === 'student' ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', paddingLeft: '2px' }}>
                        {(() => {
                          ...
                          const todayStr = `${simNow.getFullYear()}-${String(simNow.getMonth() + 1).padStart(2, '0')}-${String(simNow.getDate()).padStart(2, '0')}`;
                          const tempDateNow = new Date(Date.UTC(simNow.getFullYear(), simNow.getMonth(), simNow.getDate()));
                          tempDateNow.setUTCDate(tempDateNow.getUTCDate() + 4 - (tempDateNow.getUTCDay() || 7));
                          const yearStartNow = new Date(Date.UTC(tempDateNow.getUTCFullYear(), 0, 1));
                          const currentSimWeekNum = Math.ceil((((tempDateNow.getTime() - yearStartNow.getTime()) / 86400000) + 1) / 7);
                          const currentSimMonthKey = `${simNow.getFullYear()}-${String(simNow.getMonth() + 1).padStart(2, '0')}`;
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                              ...
                              {sortedWeekKeys.map((weekKey, weekIdx) => {
                                ...
                                const isWeekExpanded = expandedWeeks[weekKey] !== undefined
                                  ...
                                  : (isCurrentWeek || (monthKey === currentSimMonthKey && weekIdx === 0) || weekIdx === 0);
                                ...
                                return (
                                  <div key={weekKey} style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    ...
                                    <div
                                      ...
                                      aria-label={`Kalenderwoche ${wGroup.weekNum} ${isWeekExpanded ? 'einklappen' : 'ausklappen'}`}
                                      onKeyDown={(e) => {
                                        if (e.key === 'Enter' || e.key === ' ') {
                                          e.preventDefault();
                                          setExpandedWeeks(prev => ({ ...prev, [weekKey]: !isWeekExpanded }));
                                        }
                                      }}
  ...
  const renderTeacherEventPlanningColumn = () => {
    const appNow = getEffectiveAppDate();
    const todayStr = `${appNow.getFullYear()}-${String(appNow.getMonth() + 1).padStart(2, '0')}-${String(appNow.getDate()).padStart(2, '0')}`;
  ...
  return (
    <div style={{
      ...
      {isMobilePortrait ? (
        ...
        <>
          {/* COLUMN 1: Unterrichtstermine */}
          {showLessons ? renderLessonsColumn() : renderTimelineColumn()}

          {/* COLUMN 2: Schul-Events & Termine */}
          {showLessons ? renderTimelineColumn() : (role === 'secretary' ? renderAnnouncementsColumn() : renderTeacherEventPlanningColumn())}

          {/* COLUMN 3: Hier entsteht eine neue Funktion (In Vorbereitung) */}
          {renderFeatureComingSoonColumn(role === 'student')}
        </>
      )}
