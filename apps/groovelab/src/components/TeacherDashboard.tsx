...
const TeacherFeedWidget = lazy(() => import('./teacher/TeacherFeedWidget').then(m => ({ default: m.TeacherFeedWidget })));
const TeacherHausaufgabenWidget = lazy(() => import('./teacher/TeacherHausaufgabenWidget').then(m => ({ default: m.TeacherHausaufgabenWidget })));
const TeacherTagesplanWidget = lazy(() => import('./teacher/TeacherTagesplanWidget').then(m => ({ default: m.TeacherTagesplanWidget })));
...
export function TeacherDashboard({ 
    ...
    try {
      ...
      if (occId && !String(occId).startsWith('virtual-') && !String(occId).startsWith('virt_')) {
        ...
        await supabase.from('schedule_occurrences').update({
          ...
          notes: null,
          updated_at: new Date().toISOString()
        }).eq('schedule_id', scheduleId).eq('date', dateStr);
      }
      ...
      const reactivationText = `🔄 Termin reaktiviert: Dein Unterrichtstermin am ${shortDay} ${shortDate} um ${timeLabel} Uhr findet regulär statt.\n🕒 Reaktiviert am: ${execTimestampStr} durch Lehrkraft (${teacherDisplayName}).`;
  ...
  const handleReportAbsence = async () => {
    ...
    try {
      ...
      // ⚡ 1. VERSUCH: Atomarer High-Speed RPC (Ausführung in < 100ms)
      ...
      // 2. FALLBACK: Falls Migration noch nicht deployt ist
      if (!rpcSuccess) {
        ...
        // Fetch weekly schedules & occurrences
            ...
            if (startDateTime >= now) {
              ...
              if (isCurrentlyAbsent) {
                ...
                if (sched.student_id) {
                  ...
                  // 💬 Termingekoppelte 1:1 Shoutbox-Nachricht
  ...
  const handleEndAbsence = async () => {
    ...
    try {
      ...
      // 📲 Reaktivierungs-Pushs an Schüler senden
  ...
  useEffect(() => {
    if (!showAbsenceModal && !absenceNotifModal && !openAnnouncementDetailModal && !showAbsenceOverviewModal && !showAbsenceEndedModal) return;
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (showAbsenceEndedModal) setShowAbsenceEndedModal(false);
        else if (openAnnouncementDetailModal) setOpenAnnouncementDetailModal(null);
  ...
  }, [showAbsenceModal, absenceNotifModal, openAnnouncementDetailModal, showAbsenceOverviewModal, showAbsenceEndedModal]);
  ...
  const activeAbsenceCancellations = useMemo(() => {
    ...
    // 1. Primärquelle: Krisen-Benachrichtigungen mit volljuristischem Status (§ 130 BGB)
    (crisisNotifications || []).forEach((n: any) => {
      ...
      // Strikt zeitpunktbezogene Filterung: Keine Stunden vor dem Abmelde-Zeitpunkt berücksichtigen
    ...
    // 2. Sekundärquelle: Lokale Briefing-Timeline für aktuelle Ausfälle
  ...
  // State für eingeklappte Tages-Akkordeons (standardmäßig alle ausgeklappt)
  const [collapsedAbsenceDates, setCollapsedAbsenceDates] = useState<Record<string, boolean>>({});

  const toggleAbsenceDateCollapse = (dateKey: string) => {
           ...
           if (combinedOccurrences && combinedOccurrences.length > 0) {
             combinedOccurrences.forEach((occ: any) => {
               ...
               if (occ.original_date === todayStr && occ.date !== todayStr) {
                 ...
                 const occStatus = occ.status || 'scheduled';
                 const mappedItem = {
                   id: occ.id,
    ...
    try {
      ...
      if (tData?.school_id) {
        ...
        let wallSongsQuery = supabase.from('songs').select(`
          id, artist, title, media_link, tomplay_url, instrumentation,
          user_song_skills (
        ...
        const isGrooveLabMode = activePlatform === 'groovelab';
        ...
        const [
          ...
          (activeTab === 'live' || activeTab === 'briefing' || activeTab === 'coaches')
            ...
            : Promise.resolve({ data: [], error: null }),
          // submissions (user_song_skills pending approval) - Strictly GrooveLab
          ...
          ((activeTab === 'live' || activeTab === 'briefing') && viewMode !== 'student')
            ...
            : Promise.resolve({ data: null, error: null }),
          // forming bands - Strictly GrooveLab
          (isGrooveLabMode && (activeTab === 'briefing' || activeTab === 'live'))
            ...
            : Promise.resolve({ data: [], error: null }),
          // wall songs - Strictly GrooveLab
          (isGrooveLabMode && (activeTab === 'briefing' || activeTab === 'proposals' || activeTab === 'live'))
            ? Promise.resolve(wallSongsQuery).catch(e => ({ data: [], error: e }))
            : Promise.resolve({ data: [], error: null }),
          // occupied slots (band_song_slots) - Strictly GrooveLab
          (isGrooveLabMode && (activeTab === 'briefing' || activeTab === 'live'))
...
useEffect(() => {
  ...
  const renderAbsenceCardWidget = () => {
    ...
    if (!isAbsent) {
      return (
        ...
        >
          ...
          >
            <CalendarX size={14} strokeWidth={2.4} />
    ...
    // Wenn isAbsent = true: 2-Zeilen Apple HIG Control Card (Null Überlappung)
    return (
      <div 
        className="absence-card-container"
        style={{ 
          padding: '16px', 
          borderRadius: '24px',
          background: 'linear-gradient(135deg, #fff5f5 0%, #ffffff 100%)',
          boxShadow: '0 4px 18px -2px rgba(239, 68, 68, 0.12), 0 1px 3px rgba(0, 0, 0, 0.04)',
      ...
      >
        {/* Zeile 1: Status & Schnellzugriff auf Ausfall-Übersicht (§ 130 BGB) */}
        <div
          role="button"
          tabIndex={0}
          aria-label="Ausfall-Status und Kenntnisnahmen einsehen"
          onClick={() => setShowAbsenceOverviewModal(true)}
          onKeyDown={(e) => {
            if (e.key === 'Enter' || e.key === ' ') {
              e.preventDefault();
              setShowAbsenceOverviewModal(true);
            }
          }}
          style={{
            display: 'flex',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              ...
              <p style={{ 
                ...
                Bis {absenceUntilFormatted} • {totalAbsenceCancellationsCount} {totalAbsenceCancellationsCount === 1 ? 'Ausfall' : 'Ausfälle'}
              </p>
            </div>
          </div>
        ...
        {/* Zeile 2: Symmetrisches 50/50 Apple Control Grid (Garantierte Überlappungsfreiheit) */}
        <div style={{
          display: 'grid',
          ...
          {/* Button 1: Anpassen (Weiß mit feiner roter Outline) */}
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
          ...
          >
            <Calendar size={14} strokeWidth={2.4} color="#dc2626" />
            <span>Anpassen</span>
          </button>

          {/* Button 2: Wieder verfügbar (Campus-Grün mit weißem Text) */}
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
            ...
            style={{
              ...
              justifyContent: 'center',
              gap: '6px',
              boxShadow: '0 3px 12px rgba(52, 168, 83, 0.32)',
              transition: 'all 0.15s ease'
            }}
            onMouseOver={e => {
              if (!submittingAbsence) e.currentTarget.style.transform = 'scale(1.02)';
            }}
            onMouseOut={e => {
          ...
          >
            ...
            <span>{submittingAbsence ? 'Wird beendet...' : 'Wieder verfügbar'}</span>
          </button>
        </div>
      </div>
    );
    ...
    <aside style={{
          ...
          <Suspense fallback={<div style={{ padding: "40px", textAlign: "center", color: "#64748b" }}>Live Lab wird geladen...</div>}>
            <TeacherLiveView
              ...
              unreadShouts={unreadShouts}
              submissions={submissions}
              allSubmissions={allSubmissions}
              setShowAllSubmissions={setShowAllSubmissions}
              wallSongs={wallSongs}
              rehearsalSuggestions={rehearsalSuggestions}
              ...
              showKioskView={showKioskView}
              setShowKioskView={setShowKioskView}
              showKioskPinSetup={showKioskPinSetup}
              setShowKioskPinSetup={setShowKioskPinSetup}
              kioskPinInput={kioskPinInput}
              setKioskPinInput={setKioskPinInput}
              targetKioskStation={targetKioskStation}
              ...
              handleResolveHelp={handleResolveHelp}
        ...
        <Suspense fallback={<div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Schülerverwaltung wird geladen...</div>}>
          <TeacherStudentsView
            ...
            studentInstrumentFilter={studentInstrumentFilter}
            setStudentInstrumentFilter={setStudentInstrumentFilter}
            showRealNames={showRealNames}
            toggleRealNames={toggleRealNames}
