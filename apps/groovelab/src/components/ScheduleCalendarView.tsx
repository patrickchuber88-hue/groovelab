...
export function ScheduleCalendarView({ 
  ...
  useEffect(() => {
    ...
    const loadUserPreferences = async () => {
      try {
        ...
        console.warn('[ScheduleCalendarView] Error loading preferred_room_ids:', err);
      }
    };
    loadUserPreferences();
    ...
    setFavoriteRoomIds(prev => {
      ...
      if (userId) {
        supabase
          ...
          .then(({ error }) => {
            if (error) {
              console.warn('[ScheduleCalendarView] Failed to sync preferred_room_ids:', error);
            }
          });
      }
      return updated;
      ...
      for (const singleUrl of urls) {
        try {
          ...
          try {
            ...
            console.warn('[ScheduleCalendarView] Direct calendar sync failed or timed out (2.5s), skipping unreachable external feed:', singleUrl);
            continue;
          }
    ...
    try {
      ...
      for (const change of changesToSave) {
        ...
        if (isGroupBlock && (change as any).groupOccurrences && (change as any).groupOccurrences.length > 0) {
          ...
          // If change.id is an existing database record UUID, update directly
          const isRealUuid = change.id && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(change.id);
          if (isRealUuid) {
  ...
  async function loadOccurrences() {
    ...
    try {
      ...
      // Only keep occurrences whose active date is within the viewed week for rendering in the calendar grid
      // (occurrences where only original_date was in this week were loaded to satisfy hasDbRecordForThisSlot and prevent ghost projection)
              ...
              {loading && dayOccurrences.length === 0 ? (
                ...
                return renderedGroups.map(group => {
                  ...
                  if (isCancelled || isAbsentSlot) {
                    ...
                    // Exclusive Golden-Yellow Palette for Tausch-Termine (OWASP / Goldstandard):
                    // Clearly separated from Gruppentermine (#0284c7 Blau)
    ...
    {showMobileToolsSheet && (
      ...
      >
        ...
        >
          ...
          {/* Section 3: Tauschen & Gruppen */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>⇄ Tauschen & Gruppen</span>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <button
                onClick={() => {
                  const nextActive = !isSwapModeActive;
                  setIsSwapModeActive(nextActive);
                  if (!nextActive) setSwapSourceOcc(null);
                  setShowMobileToolsSheet(false);
                }}
                style={{ padding: '12px', borderRadius: '14px', border: isSwapModeActive ? '1.5px solid #eab308' : '1px solid #fde047', background: isSwapModeActive ? '#fefce8' : '#f8fafc', color: isSwapModeActive ? '#854d0e' : '#334155', fontWeight: 800, fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}
              >
