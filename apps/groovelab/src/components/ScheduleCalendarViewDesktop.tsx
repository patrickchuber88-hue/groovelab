...
export function ScheduleCalendarViewDesktop({ 
  schoolId, 
  userId, 
  boards, 
  activeTab, 
  ...
  // Apple HIG Stepped Edge-Resize state (strictly for breaks)
  const [resizingBreakOcc, setResizingBreakOcc] = useState<{
    id: string;
  ...
  const showActionToast = (message: string, undoFn?: () => void, skipVisualToastIfStaging: boolean = false) => {
    ...
    if (skipVisualToastIfStaging && Object.keys(pendingChanges).length > 0) {
      return;
    }
    setActionToast({
      id: String(Date.now()),
  ...
  // Beim Öffnen des Stundenplan-Boards sicherstellen, dass die Wochentagsleiste bündig im sichtbaren Blickfeld verankert ist
  ...
  const holidaysFingerprint = useMemo(() => {
    return (holidays || []).map(h => `${h.start}_${h.end}`).join('|');
  }, [holidays]);
  ...
  async function loadOccurrences() {
    ...
    try {
      ...
      try {
        const [
          ...
          supabase.from('schedule_occurrences')
            .select('id, date, start_time, original_date, duration, status, notes, teacher_id, student_id, schedule_id, template_room_id, room_override_id, student:users!schedule_occurrences_student_id_fkey(id, school_id, teacher_id, first_name, last_name, instrument, is_campus_active, is_groovelab_active, group_id, campus_ui_level, parent_permissions), schedules:schedules(room_id, room:rooms(name))')
            .eq('teacher_id', userId)
      ...
      // Live Calendar Projection Isolation:
      // Only project template slots if they belong to an approved schedule.
      // If a new draft is pending review, the live calendar exclusively renders existing database occurrences.
      const projectedData: ScheduleOccurrence[] = [];
      const isEligibleForTemplateProjection = Boolean(boards && boards.length > 0);
      if (isEligibleForTemplateProjection) {
        boards.forEach(board => {
          const offset = board.dayOfWeek - 1;
      ...
      // Only include occurrences whose active date falls in this week for grid display
      fetchedData = fetchedData.filter(o => o.date >= startDateStr && o.date <= endDateStr);
  ...
  }, [currentDate, occurrences, pendingChanges, weekStart, isResetModalOpen, isSwapModeActive, actionToast]);
  ...
  const executeOccurrenceSwap = async (sourceId: string, targetId: string) => {
    ...
    srcGroupOccs.forEach(o => {
      updatesMap[o.id] = { 
        ...
        student_acknowledged: false,
        is_swap: true,
        notes: `[Tauschtermin] Getauscht mit ${tgtName}`
      };
    });
    tgtGroupOccs.forEach(o => {
      updatesMap[o.id] = { 
        ...
        student_acknowledged: false,
        is_swap: true,
        notes: `[Tauschtermin] Getauscht mit ${srcName}`
      };
    });
  ...
  const handleTwoClickSwap = async (sourceOcc: ScheduleOccurrence, targetOcc: ScheduleOccurrence) => {
    if (sourceOcc.id === targetOcc.id) {
      setSwapSourceOcc(null);
      return;
    }

    if (!sourceOcc.student_id || sourceOcc.student_id === 'vacant' || !targetOcc.student_id || targetOcc.student_id === 'vacant') {
      await showAlert('Nur belegte Schüler-Termine können miteinander getauscht werden.');
      return;
    }
    ...
    const sourceConflict = getRoomConflict(sourceOcc.id, targetOcc.date, targetOcc.start_time, sourceOcc.duration, sourceRoomId, targetOcc.id);
    const targetConflict = getRoomConflict(targetOcc.id, sourceOcc.date, sourceOcc.start_time, targetOcc.duration, targetRoomId, sourceOcc.id);

    if (sourceConflict || targetConflict) {
      let conflictsText = '';
      if (sourceConflict) {
        const roomName = sourceOcc.schedules?.room?.name || 'Raum';
        conflictsText += `\n- Für ${sourceOcc.student?.first_name || 'Schüler 1'} in Raum "${roomName}": belegt durch ${sourceConflict}`;
      }
      if (targetConflict) {
        const roomName = targetOcc.schedules?.room?.name || 'Raum';
        conflictsText += `\n- Für ${targetOcc.student?.first_name || 'Schüler 2'} in Raum "${roomName}": belegt durch ${targetConflict}`;
      }
    ...
    const sourceOrigDate = sourceOcc.original_date || sourceOcc.date;
    const sourceOrigTime = sourceOcc.original_start_time || sourceOcc.start_time;
    const targetOrigDate = targetOcc.original_date || targetOcc.date;
    const targetOrigTime = targetOcc.original_start_time || targetOcc.start_time;

    const srcName = `${sourceOcc.student?.first_name || 'Schüler 1'} ${maskLastName(sourceOcc.student?.last_name || '', showRealNames)}`.trim();
    const tgtName = `${targetOcc.student?.first_name || 'Schüler 2'} ${maskLastName(targetOcc.student?.last_name || '', showRealNames)}`.trim();

    const sourceUpdated: ScheduleOccurrence = {
      ...sourceOcc,
      date: targetOcc.date,
      start_time: targetOcc.start_time,
      original_date: sourceOrigDate,
      original_start_time: sourceOrigTime,
      duration: sourceOcc.duration,
      status: 'pending_reschedule',
      student_acknowledged: false,
      is_swap: true,
      notes: `[Tauschtermin] Getauscht mit ${tgtName}`
    ...
    const targetUpdated: ScheduleOccurrence = {
      ...targetOcc,
      date: sourceOcc.date,
      start_time: sourceOcc.start_time,
      original_date: targetOrigDate,
      original_start_time: targetOrigTime,
      duration: targetOcc.duration,
      status: 'pending_reschedule',
      student_acknowledged: false,
      is_swap: true,
    ...
    try {
      await persistChangesDirectly([sourceUpdated, targetUpdated]);
      ...
      showActionToast(`${srcName} und ${tgtName} erfolgreich getauscht!`, async () => {
        const sourceReverted: ScheduleOccurrence = {
          ...
          original_date: sourceOrigDate,
          original_start_time: sourceOrigTime,
          status: sourceOcc.status || 'scheduled'
        };
        const targetReverted: ScheduleOccurrence = {
          ...targetOcc,
          date: targetOcc.date,
          start_time: targetOcc.start_time,
          original_date: targetOrigDate,
          original_start_time: targetOrigTime,
          status: targetOcc.status || 'scheduled'
        };
        await persistChangesDirectly([sourceReverted, targetReverted]);
        await loadOccurrences();
      });
      ...
      console.error('Error swapping occurrences:', err);
      await showAlert('Fehler beim Tauschen der Termine: ' + (err.message || err));
    }
  };
  ...
  const handleDropOnOccurrence = async (e: React.DragEvent, targetId: string) => {
    ...
    // 🍏 Apple HIG Direct Manipulation: Dropping directly executes 1:1 swap without modal blockage.
    // Holding Alt/Option key (e.altKey) allows teachers to open the merge decision dialog if desired.
    if (e.altKey) {
    ...
    setDraggedId(null);
    await executeOccurrenceSwap(sourceId, targetId);
  };
  ...
  const handleCancel = async (e: React.MouseEvent, id: string, studentName?: string) => {
    e.stopPropagation();
    const confirmed = await showConfirm(
      studentName
        ? `Möchtest du den Termin für ${studentName} absagen? Der Termin bleibt im Kalender rot/weiß gestreift als "Abgesagt" dokumentiert.`
        : 'Möchtest du diesen Termin absagen? Der Termin bleibt im Kalender rot/weiß gestreift als "Abgesagt" dokumentiert.'
    );
    if (!confirmed) return;
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            <div id="tour-calendar-actions" className="apple-btn-group">
              ...
              <button
                ...
                className="apple-btn"
                aria-label="Stundenplan-Termine auf die Stammdaten des Stundenplan-Designers zurücksetzen"
                style={{ color: '#ef4444' }}
      ...
      {isSwapModeActive && (
        <div
          style={{
            ...
            padding: '10px 18px',
            borderRadius: '20px',
            boxShadow: '0 12px 28px -6px rgba(0, 0, 0, 0.35), 0 0 0 1px rgba(255, 255, 255, 0.1)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'space-between',
            gap: '14px'
          ...
          return (
            ...
            >
              <div 
                ...
                onMouseOver={e => e.currentTarget.style.background = 'rgba(248, 250, 252, 0.98)'}
              ...
              {loading && dayOccurrences.length === 0 ? (
                ...
                return renderedGroups.map(group => {
                  ...
                  const isGroup = occurrencesInGroup.length > 1 || Boolean(occ.isGroupBlock) || Boolean((occ as any).groupOccurrences?.length > 0) || Boolean(getTemplateGroupForOcc(occ, offset + 1, boards));
                  ...
                  if (isCancelled || isAbsentSlot) {
                    ...
                    if (isSwap) {
                      // Campus Swapped Lessons Color Palette (Signature Golden Amber):
                      if (isWaiting) {
                        // Gelb gestreift = Tauschtermin ausstehend / unbestätigt
                        ...
                        // Vollton Sonnengelb = Tauschtermin bestätigt
                  ...
                  let regularDayMin = Infinity;
                  let regularDayMax = -Infinity;

                  if (boards && boards.length > 0) {
                    const dayBoard = boards.find((b: any) => b.dayOfWeek === dayOfWeek);
                    if (dayBoard && Array.isArray(dayBoard.students)) {
                      dayBoard.students.forEach((s: any) => {
                        const timeStr = s.assignedTime || s.customStartTime;
                        if (timeStr) {
                          const start = timeToMinutes(timeStr);
                          const duration = s.duration || 30;
                          const end = start + duration;
                          if (start < regularDayMin) regularDayMin = start;
                          if (end > regularDayMax) regularDayMax = end;
                        }
                      });
                    }
                  }
                  ...
                  // 2. Secondary fallback: cachedWeekSchedules
                  if (regularDayMin === Infinity && cachedWeekSchedules && cachedWeekSchedules.length > 0) {
                    cachedWeekSchedules.forEach((s: any) => {
                      if (s.day_of_week === dayOfWeek && s.teacher_id === userId && s.time_slot) {
                        const start = timeToMinutes(s.time_slot);
                        const duration = s.duration || 45;
                        const end = start + duration;
                        if (start < regularDayMin) regularDayMin = start;
                  ...
                  const hasRegularDayBlock = regularDayMin !== Infinity;

                  // An appointment is ONLY outside schedule if:
                  // 1. It is NOT a regular Stammtermin at its master slot (isAtMasterSlot === false), AND
                  // 2. It was moved outside the teacher's regular teaching window, OR has an explicit extra room booking requested.
                  const isExplicitExtraRoomBooking = Boolean(
                    ...
                    <React.Fragment key={group.key}>
                      <div 
                        ...
                        style={{ 
                          ...
                          height: `${Math.max(28, ((isBreak && resizingBreakOcc?.id === occ.id ? (resizingBreakOcc?.currentDuration || (occ.duration || 15)) : (occ.duration || (isBreak ? 15 : 30))) * 2.5 - 8))}px`,
                          minHeight: '28px',
                          flexShrink: 0,
                          boxSizing: 'border-box',
                          overflow: 'hidden'
                      ...
                      >
                        {(() => {
                          const duration = (isBreak && resizingBreakOcc?.id === occ.id) 
                            ? (resizingBreakOcc?.currentDuration || (occ.duration || 15))
                            : (occ.duration || (isBreak ? 15 : 30));
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', height: '100%', width: '100%', justifyContent: 'space-between', padding: '1px 0', gap: '2px' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4px', width: '100%', overflow: 'hidden' }}>
                                  ...
                                  {(isCancelled || isAbsentSlot) ? (
                                    ...
                                    <span style={{ fontSize: '0.62rem', fontWeight: 700, opacity: 0.6, flexShrink: 0 }}>
                                      {occ.duration || 20}m
                                    </span>
                                  )}
                                </div>
...
return (
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                  ...
                                  {(() => {
                                    ...
                                    try {
                                      ...
                                      if (occ.date !== todayStr) return null;
                                      const timeStr = occ.start_time ? occ.start_time.substring(0, 5) : '';
                                      if (!timeStr) return null;
                                      const [h, m] = timeStr.split(':').map(Number);
                                      const apptTime = new Date(today);
                                      apptTime.setHours(h, m, 0, 0);
                                      const diff = Math.round((apptTime.getTime() - today.getTime()) / 60000);
                                      if (diff <= 120 && diff >= -30) {
                                        return (
                                          <span
                                            style={{
                              ...
                              {isGroupLesson ? (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', width: '100%' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.78rem', fontWeight: 800, color: finalColors.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                    ...
                                    <span style={{ fontSize: '0.65rem', fontWeight: 700, opacity: 0.6, background: 'rgba(0,0,0,0.05)', padding: '1px 5px', borderRadius: '4px', marginLeft: 'auto' }}>
                                      {occ.duration || 45}m
                                    </span>
                                  </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4px' }}>
                                    ...
                                    <span style={{ fontSize: '0.65rem', fontWeight: 700, opacity: 0.6, background: 'rgba(0,0,0,0.05)', padding: '1px 5px', borderRadius: '4px', flexShrink: 0 }}>
                                      {occ.duration || 45}m
                                    </span>
                                  </div>
                                  {isBreak ? (
                        ...
                        {/* 🍏 Apple HIG Edge-Resize Handle: Ausschließlich für Pausenkarten (Unterrichtsdauer von Schülern ist vertraglich fixiert & unantastbar) */}
                        {isBreak && occ.status !== 'cancelled' && (
                          <div
                            role="separator"
                            aria-orientation="horizontal"
                            aria-label={`Pausenlänge anpassen (aktuell ${occ.duration || 15} Minuten)`}
                            tabIndex={0}
                            onPointerDown={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              ...
                              setResizingBreakOcc({
                                id: occ.id,
                                initialDuration: occ.duration || 15,
                                startY: e.clientY,
                                currentDuration: occ.duration || 15
                              });
                            }}
                            onPointerMove={(e) => {
                              if (!resizingBreakOcc || resizingBreakOcc.id !== occ.id) return;
                              e.preventDefault();
                              e.stopPropagation();
                              const deltaPx = e.clientY - resizingBreakOcc.startY;
                              const deltaMin = Math.round((deltaPx / 2.5) / 15) * 15;
                              const newDuration = Math.min(120, Math.max(15, resizingBreakOcc.initialDuration + deltaMin));
                              if (newDuration !== resizingBreakOcc.currentDuration) {
                                setResizingBreakOcc(prev => prev ? { ...prev, currentDuration: newDuration } : null);
                              }
                            }}
                            onPointerUp={(e) => {
                              if (!resizingBreakOcc || resizingBreakOcc.id !== occ.id) return;
                              ...
                              const finalDuration = resizingBreakOcc.currentDuration;
                              setResizingBreakOcc(null);
                              if (finalDuration !== (occ.duration || 15)) {
                                updateOccurrence(occ.id, { duration: finalDuration }, `Pause auf ${finalDuration} Min. angepasst`);
                              }
                            }}
                            onPointerCancel={(e) => {
                              if (!resizingBreakOcc || resizingBreakOcc.id !== occ.id) return;
                            ...
                            onKeyDown={(e) => {
                              const currentDur = occ.duration || 15;
                              if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
                                e.preventDefault();
                                const newDuration = Math.min(120, currentDur + 15);
                                if (newDuration !== currentDur) {
                                  updateOccurrence(occ.id, { duration: newDuration }, `Pause auf ${newDuration} Min. verlängert`);
                                }
                              } else if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
                                e.preventDefault();
                                const newDuration = Math.max(15, currentDur - 15);
                                if (newDuration !== currentDur) {
                                  updateOccurrence(occ.id, { duration: newDuration }, `Pause auf ${newDuration} Min. verkürzt`);
                                }
                              }
                            }}
                            title="Pausenlänge durch Ziehen nach unten/oben oder Pfeiltasten anpassen (15m-Schritte)"
                            style={{
                              position: 'absolute',
                              left: 0,
                              right: 0,
                          ...
                          >
                            <div style={{
                              ...
                              height: '3px',
                              borderRadius: '2px',
                              background: resizingBreakOcc?.id === occ.id ? '#ea580c' : 'rgba(234, 88, 12, 0.4)',
                              transition: 'all 0.15s ease',
                              boxShadow: resizingBreakOcc?.id === occ.id ? '0 0 6px rgba(234, 88, 12, 0.5)' : 'none'
                            }} />
                          </div>
                        )}
                      </div>
      ...
      {/* 🍏 Unified Floating Action Dock in Hellem Campus-Design */}
      {Object.keys(pendingChanges).length > 0 && (
        <>
          <style>{`
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '0.82rem', fontWeight: 750, color: '#0f172a', fontFamily: "'Plus Jakarta Sans', -apple-system, sans-serif" }}>
                {Object.keys(pendingChanges).length === 1 ? 'Ungespeicherte Änderung' : 'Ungespeicherte Änderungen'}
              </span>
            </div>

            {/* Actions group: Undo (⌘Z) | Verwerfen | Speichern */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <button
                type="button"
                onClick={handleUndo}
                ...
                title="Letzte Verschiebung schrittweise rückgängig machen (⌘Z)"
                aria-label="Letzte Verschiebung rückgängig machen (⌘Z)"
              >
                <RotateCcw size={13} strokeWidth={2.4} />
                <span>Rückgängig (⌘Z)</span>
              </button>
    ...
    {/* ========================================================================= */}
    {/* ENTERPRISE+ GOLDSTANDARD: RESET SCHEDULE MODAL (MINIMALIST APPLE SHEET)   */}
    {/* ========================================================================= */}
    {isResetModalOpen && (
      <div
        style={{
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <div>
                <h3 id="reset-modal-title" style={{ margin: 0, fontSize: '1.05rem', fontWeight: 850, color: '#0f172a', letterSpacing: '-0.02em' }}>
                  Stundenplan zurücksetzen
                </h3>
                <p style={{ margin: 0, fontSize: '0.74rem', color: '#64748b', fontWeight: 500 }}>
                  Stellt die regulären Zeiten des Designers wieder her
                </p>
              </div>
            </div>
            <button
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '0.80rem', fontWeight: 800, color: '#991b1b' }}>
                    {resetProgress.stage || 'Bereinigung läuft...'}
                  </span>
                </div>
                <span style={{ fontSize: '0.82rem', fontWeight: 900, color: '#dc2626', fontVariantNumeric: 'tabular-nums' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', opacity: isExecutingReset ? 0.55 : 1, pointerEvents: isExecutingReset ? 'none' : 'auto' }}>
            {/* Option 1: Nur diese Kalenderwoche */}
            <div
              role="button"
              ...
              onMouseEnter={e => {
                if (!isExecutingReset) e.currentTarget.style.borderColor = '#94a3b8';
              }}
              onMouseLeave={e => {
                if (!isExecutingReset) e.currentTarget.style.borderColor = '#e2e8f0';
              }}
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                <span style={{ fontSize: '0.88rem', fontWeight: 800, color: '#0f172a' }}>
                  Nur diese Woche (KW {getWeekNumber(weekStart)})
                </span>
                <span style={{ fontSize: '0.72rem', color: '#64748b' }}>
                  {resetImpact.loading 
                    ? 'Prüfe Abweichungen...' 
                    : `${resetImpact.weekCount} abweichende Termine (${resetImpact.weekStudentCount} Schüler)`
                  }
                </span>
              </div>
              <div
            ...
            <div
              ...
              onMouseEnter={e => {
                if (!isExecutingReset) e.currentTarget.style.borderColor = '#f87171';
              }}
              onMouseLeave={e => {
                if (!isExecutingReset) e.currentTarget.style.borderColor = '#fecaca';
              }}
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontSize: '0.72rem', color: '#b91c1c' }}>
                  {resetImpact.loading 
                    ? 'Prüfe Abweichungen...' 
                    : `${resetImpact.schoolYearCount} abweichende Termine (${resetImpact.schoolYearStudentCount} Schüler)`
                  }
                </span>
              </div>
              <div
              ...
              >
                {isExecutingReset ? `${resetProgress.percent}%` : 'Bereinigen'}
              </div>
            </div>
          </div>

          {/* Subtiler Footer-Hinweis: Historien- & Designer-Schutz */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', paddingTop: '2px' }}>
            <ShieldCheck size={14} color="#16a34a" />
            <span style={{ fontSize: '0.70rem', color: '#64748b', fontWeight: 600 }}>
              Vergangene Termine vor heute bleiben unberührt.
            </span>
          </div>
        </div>
      </div>
    ...
    {/* 🍏 Apple HIG Floating Action Toast with Dynamic Stacking Clearance & Light Campus Design */}
    {actionToast && (
      <div
        role="status"
        aria-live="polite"
        style={{
          position: 'fixed',
          bottom: Object.keys(pendingChanges).length > 0 ? '96px' : '24px',
          left: '50%',
          transform: 'translateX(-50%)',
          background: 'rgba(255, 255, 255, 0.96)',
          backdropFilter: 'blur(20px) saturate(190%)',
          ...
          padding: '8px 16px',
          borderRadius: '9999px',
          boxShadow: '0 12px 36px -4px rgba(0, 0, 0, 0.12), 0 2px 10px rgba(0, 0, 0, 0.04)',
          display: 'flex',
          alignItems: 'center',
          gap: '12px',
          zIndex: 99999,
      ...
      >
        <span style={{ fontWeight: 750, color: '#0f172a' }}>{actionToast.message}</span>
        {actionToast.undoFn && (
          <button
            type="button"
            onClick={() => handleUndo()}
            aria-label="Aktion rückgängig machen (⌘Z)"
            style={{
              background: '#f0fdf4',
              border: '1px solid #bbf7d0',
              color: '#166534',
