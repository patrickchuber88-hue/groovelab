...
export const TeacherDetailModal: React.FC<TeacherDetailModalProps> = ({ teacher, onClose }) => {
  ...
  const TEACHER_MODAL_INSTRUMENT_ICONS: Record<string, string> = { Guitar: '🎸', Bass: '🎸', Drums: '🥁', Keys: '🎹' };
  ...
  // Helper to safely parse arrays from DB (handles JSON strings or comma-separated strings)
  const parseArray = (val: any) => {
    ...
    if (typeof val === 'string') {
      try {
        ...
        // Fallback: split by comma if it's just a normal string
  ...
  const bands = parseArray(teacher.bands);
  const projects = parseArray(teacher.projects);
  const musicalStyles = parseArray(teacher.musical_styles);
      ...
      <div className="glass-panel animation-slide-up" style={{ background: 'white', padding: '32px', borderRadius: '32px', maxWidth: '500px', width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <h2 style={{ fontSize: '1.75rem', fontWeight: 800, margin: 0, color: '#1e293b' }}>
            {teacher.first_name} {teacher.last_name ? `${teacher.last_name[0]}.` : ''}
          </h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <section>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              {musicalStyles.map((style: string, idx: number) => (
              ...
              {musicalStyles.length === 0 && <div style={{ fontSize: '0.85rem', color: '#cbd5e1', fontStyle: 'italic' }}>Keine Expertise eingetragen</div>}
            </div>
          </section>
